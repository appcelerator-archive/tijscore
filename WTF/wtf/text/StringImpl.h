...
class StringImpl {
    ...
    friend struct TI::IdentifierASCIIStringTranslator;
    friend struct TI::IdentifierCharBufferTranslator<LChar>;
    friend struct TI::IdentifierCharBufferTranslator<UChar>;
    friend struct TI::IdentifierLCharFromUCharTranslator;
    friend struct WTI::CStringTranslator;
    template<typename CharacterType> friend struct WTI::HashAndCharactersTranslator;
    friend struct WTI::HashAndUTF8CharactersTranslator;
    friend struct WTI::CharBufferFromLiteralDataTranslator;
    friend struct WTI::LCharBufferTranslator;
    friend struct WTI::SubstringTranslator;
    friend struct WTI::UCharBufferTranslator;
    friend class AtomicStringImpl;
    friend class TI::LLInt::Data;
    friend class TI::LLIntOffsetsExtractor;
...
{
    if (is8Bit())
        return WTI::find(characters8(), m_length, character, start);
    return WTI::find(characters16(), m_length, character, start);
}
...
using WTI::equal;
using WTI::equalNonNull;
using WTI::TextCaseSensitivity;
using WTI::TextCaseSensitive;
using WTI::TextCaseInsensitive;
