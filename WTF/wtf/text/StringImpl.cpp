...
{
    if (is8Bit())
        return WTI::find(characters8(), m_length, matchFunction, start);
    return WTI::find(characters16(), m_length, matchFunction, start);
}
...
size_t StringImpl::find(const LChar* matchString, unsigned index)
{
    ...
    if (matchLength == 1)
        return WTI::find(characters16(), length(), *matchString, index);
    ...
    if (matchLength == 1) {
        if (is8Bit()) {
            if (matchString->is8Bit())
                return WTI::find(characters8(), length(), matchString->characters8()[0]);
            return WTI::find(characters8(), length(), matchString->characters16()[0]);
        }
        if (matchString->is8Bit())
            return WTI::find(characters16(), length(), matchString->characters8()[0]);
        return WTI::find(characters16(), length(), matchString->characters16()[0]);
    }
...
size_t StringImpl::find(StringImpl* matchString, unsigned index)
{
    ...
    if (matchLength == 1) {
        if (is8Bit())
            return WTI::find(characters8(), length(), (*matchString)[0], index);
        return WTI::find(characters16(), length(), (*matchString)[0], index);
    }
...
{
    if (is8Bit())
        return WTI::findNextLineStart(characters8(), m_length, index);
    return WTI::findNextLineStart(characters16(), m_length, index);
}

size_t StringImpl::reverseFind(UChar c, unsigned index)
{
    if (is8Bit())
        return WTI::reverseFind(characters8(), m_length, c, index);
    return WTI::reverseFind(characters16(), m_length, c, index);
}
...
size_t StringImpl::reverseFind(StringImpl* matchString, unsigned index)
{
    ...
    if (matchLength == 1) {
        if (is8Bit())
            return WTI::reverseFind(characters8(), ourLength, (*matchString)[0], index);
        return WTI::reverseFind(characters16(), ourLength, (*matchString)[0], index);
    }
