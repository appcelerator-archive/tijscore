...
{
    return WTI::find(m_data, m_length, c, start);
}

size_t StringImpl::find(CharacterMatchFunctionPtr matchFunction, unsigned start)
{
    return WTI::find(m_data, m_length, matchFunction, start);
}
...
size_t StringImpl::find(const char* matchString, unsigned index)
{
    ...
    if (matchLength == 1)
        return WTI::find(characters(), length(), *(const unsigned char*)matchString, index);
...
size_t StringImpl::find(StringImpl* matchString, unsigned index)
{
    ...
    // Optimization 1: fast case for strings of length 1.
    if (matchLength == 1)
        return WTI::find(characters(), length(), matchString->characters()[0], index);
...
{
    return WTI::reverseFind(m_data, m_length, c, index);
}
...
size_t StringImpl::reverseFind(StringImpl* matchString, unsigned index)
{
    ...
    // Optimization 1: fast case for strings of length 1.
    if (matchLength == 1)
        return WTI::reverseFind(characters(), length(), matchString->characters()[0], index);
...
WTI::Unicode::Direction StringImpl::defaultWritingDirection(bool* hasStrongDirectionality)
{
    for (unsigned i = 0; i < m_length; ++i) {
        WTI::Unicode::Direction charDirection = WTI::Unicode::direction(m_data[i]);
        if (charDirection == WTI::Unicode::LeftToRight) {
            if (hasStrongDirectionality)
                *hasStrongDirectionality = true;
            return WTI::Unicode::LeftToRight;
        }
        if (charDirection == WTI::Unicode::RightToLeft || charDirection == WTI::Unicode::RightToLeftArabic) {
            if (hasStrongDirectionality)
                *hasStrongDirectionality = true;
            return WTI::Unicode::RightToLeft;
        }
    }
    if (hasStrongDirectionality)
