...
namespace TI {
    ...
    {
        return isASCII(ch) ? (ch == ' ' || ch == '\t' || ch == 0xB || ch == 0xC) : (WTI::Unicode::isSeparatorSpace(ch) || ch == 0xFEFF);
    }
    ...
    ALWAYS_INLINE JSTokenType Lexer::lexExpectIdentifier(JSTokenData* tokenData, JSTokenInfo* tokenInfo, unsigned lexType, bool strictMode)
    {
        ...
        if (!WTI::isASCIIAlpha(*ptr))
            goto slowCase;
        ++ptr;
        while (ptr < end) {
            if (!WTI::isASCIIAlphanumeric(*ptr))
                break;
            ++ptr;
        }
        ...
        // Here's the shift
        if (ptr < end) {
            if ((!WTI::isASCII(*ptr)) || (*ptr == '\\') || (*ptr == '_') || (*ptr == '$'))
                goto slowCase;
            m_current = *ptr;
        } else
            m_current = -1;
