...
RegExp::RegExp(TiGlobalData* globalData, const UString& patternString, RegExpFlags flags)
    : TiCell(*globalData, globalData->regExpStructure.get())
    , m_state(NotCompiled)
...
RegExp* RegExp::create(TiGlobalData* globalData, const UString& patternString, RegExpFlags flags)
{
...
int RegExp::match(TiGlobalData& globalData, const UString& s, int startOffset, Vector<int, 32>* ovector)
{
    if (startOffset < 0)
        startOffset = 0;
