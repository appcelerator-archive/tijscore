...
struct PropertyMapEntry {
    ...
    unsigned offset;
    unsigned attributes;
    WriteBarrier<TiCell> specificValue;

    PropertyMapEntry(TiGlobalData& globalData, TiCell* owner, StringImpl* key, unsigned offset, unsigned attributes, TiCell* specificValue)
        : key(key)
        , offset(offset)
        , attributes(attributes)
        , specificValue(globalData, owner, specificValue, WriteBarrier<TiCell>::MayBeNull)
    {
    }
};
...
public:
    ...
    PropertyTable(TiGlobalData&, TiCell*, const PropertyTable&);
    PropertyTable(TiGlobalData&, TiCell*, unsigned initialCapacity, const PropertyTable&);
    ~PropertyTable();
    ...
    PassOwnPtr<PropertyTable> copy(TiGlobalData&, TiCell* owner, unsigned newCapacity);
...
inline PropertyTable::PropertyTable(TiGlobalData&, TiCell* owner, const PropertyTable& other)
...
inline PropertyTable::PropertyTable(TiGlobalData&, TiCell* owner, unsigned initialCapacity, const PropertyTable& other)
    : m_indexSize(sizeForCapacity(initialCapacity))
    , m_indexMask(m_indexSize - 1)
    , m_index(static_cast<unsigned*>(fastZeroedMalloc(dataSize())))
    , m_keyCount(0)
...
inline PropertyTable::find_iterator PropertyTable::find(const KeyType& key)
{
    ...
    while (true) {
        ...
        if (!step)
            step =WTI::doubleHash(key->existingHash()) | 1;
        hash += step;
...
inline PassOwnPtr<PropertyTable> PropertyTable::copy(TiGlobalData& globalData, TiCell* owner, unsigned newCapacity)
{
