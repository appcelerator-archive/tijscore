...
// ECMA 9.4
double TiValue::toInteger(TiExcState* exec) const
{
    if (isInt32())
        return asInt32();
...
double TiValue::toIntegerPreserveNaN(TiExcState* exec) const
{
    if (isInt32())
        return asInt32();
...
TiObject* TiValue::toObjectSlowCase(TiExcState* exec, TiGlobalObject* globalObject) const
{
    ASSERT(!isCell());
...
TiObject* TiValue::toThisObjectSlowCase(TiExcState* exec) const
{
    ASSERT(!isCell());
...
TiObject* TiValue::synthesizeObject(TiExcState* exec) const
{
    ASSERT(!isCell());
    if (isNumber())
        return constructNumber(exec, exec->lexicalGlobalObject(), asValue());
...
TiObject* TiValue::synthesizePrototype(TiExcState* exec) const
{
    ASSERT(!isCell());
    if (isNumber())
...
#ifndef NDEBUG
char* TiValue::description()
{
    static const size_t size = 32;
    static char description[size];

    if (!*this)
        snprintf(description, size, "<TiValue()>");
    else if (isInt32())
