...
namespace TI {
    ...
    class ArgList {
        friend class JIT;
    public:
        typedef TiValue* iterator;
        typedef const TiValue* const_iterator;
        ...
        ArgList(TiValue* args, unsigned argCount)
            : m_args(args)
            , m_argCount(argCount)
        {
#if ENABLE(JSC_ZOMBIES)
        ...
        ArgList(Register* args, int argCount)
            : m_args(reinterpret_cast<TiValue*>(args))
            , m_argCount(argCount)
        {
            ASSERT(argCount >= 0);
        }
        ...
        ArgList(const MarkedArgumentBuffer& args)
            : m_args(reinterpret_cast<TiValue*>(const_cast<Register*>(args.begin())))
            , m_argCount(args.size())
        {
        }

        TiValue at(size_t idx) const
        {
            if (idx < m_argCount)
                return m_args[idx];
            return jsUndefined();
