...
namespace TI {
    ...
    public:
        ...
        static MarkedBlock* create(TiGlobalData*, size_t cellSize);
    ...
    private:
        ...
        MarkedBlock(const PageAllocationAligned&, TiGlobalData*, size_t cellSize);
        Atom* atoms();

        size_t m_nextAtom;
        size_t m_endAtom; // This is a fuzzy end. Always test for < m_endAtom.
        size_t m_atomsPerCell;
        WTI::Bitmap<blockSize / atomSize> m_marks;
    ...
    template <typename Functor> inline void MarkedBlock::forEach(Functor& functor)
    {
        for (size_t i = firstAtom(); i < m_endAtom; i += m_atomsPerCell) {
            if (!m_marks.get(i))
                continue;
            functor(reinterpret_cast<TiCell*>(&atoms()[i]));
        }
    }
