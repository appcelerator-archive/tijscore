...
class QScriptConverter {
public:
    static quint32 toArrayIndex(const TiStringRef jsstring)
    {
    ...
    static QString toString(const TiStringRef str)
    {
        return QString(reinterpret_cast<const QChar*>(TiStringGetCharactersPtr(str)), TiStringGetLength(str));
    }
    static TiStringRef toString(const QString& str)
    {
        return TiStringCreateWithUTF8CString(str.toUtf8().constData());
    }
    static TiStringRef toString(const char* str)
    {
        return TiStringCreateWithUTF8CString(str);
    }
