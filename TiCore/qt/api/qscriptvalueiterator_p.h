...
private:
    ...
    QVector<TiStringRef> m_names;
    QMutableVectorIterator<TiStringRef> m_idx;
};
...
{
    QVector<TiStringRef>::const_iterator i = m_names.constBegin();
...
inline QScriptValuePrivate* QScriptValueIteratorPrivate::value() const
{
    ...
    TiValueRef value = m_object->property(m_idx.value(), &exception);
