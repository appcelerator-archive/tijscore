...
EncodedTiValue JSC_HOST_CALL Interpreter::constructWithNativeErrorConstructor(ExecState* exec)
{
    TiValue message = exec->argument(0);
    ...
    stackTrace.remove(0);
    return TiValue::encode(ErrorInstance::create(exec, errorStructure, message, stackTrace));
}
...
EncodedTiValue JSC_HOST_CALL Interpreter::callNativeErrorConstructor(ExecState* exec)
{
    TiValue message = exec->argument(0);
    Structure* errorStructure = static_cast<NativeErrorConstructor*>(exec->callee())->errorStructure();
    Vector<StackFrame> stackTrace;
