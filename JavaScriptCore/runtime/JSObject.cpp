    ...
    if (visitor.checkIfShouldCopy(butterfly->base(preCapacity, propertyCapacity))) {
        ...
        if (UNLIKELY(hasIndexingHeader)) {
            *newButterfly->indexingHeader() = *butterfly->indexingHeader();
            ...
            memcpy(currentTarget, currentSource, count * sizeof(EncodedTiValue));
        }
...
bool JSObject::getOwnPropertySlotByIndex(JSObject* thisObject, ExecState* exec, unsigned i, PropertySlot& slot)
{
    ...
    case ALL_CONTIGUOUS_INDEXING_TYPES: {
        ...
        TiValue value = butterfly->contiguous()[i].get();
        if (value) {
    ...
    case ALL_DOUBLE_INDEXING_TYPES: {
        ...
        if (value == value) {
            slot.setValue(thisObject, None, TiValue(TiValue::EncodeAsDouble, value));
            return true;
        }
    ...
    case ALL_ARRAY_STORAGE_INDEXING_TYPES: {
        ...
        if (i < storage->vectorLength()) {
            TiValue value = storage->m_vector[i].get();
            if (value) {
                slot.setValue(thisObject, None, value);
                return true;
            }
...
// ECMA 8.6.2.2
void JSObject::put(JSCell* cell, ExecState* exec, PropertyName propertyName, TiValue value, PutPropertySlot& slot)
{
    JSObject* thisObject = jsCast<JSObject*>(cell);
    ASSERT(value);
    ...
    for (obj = thisObject; ; obj = asObject(prototype)) {
        ...
        if (isValidOffset(offset)) {
            ...
            TiValue gs = obj->getDirect(offset);
...
void JSObject::putByIndex(JSCell* cell, ExecState* exec, unsigned propertyName, TiValue value, bool shouldThrow)
{
    JSObject* thisObject = jsCast<JSObject*>(cell);
...
{
    DeferGC deferGC(vm.heap);
    Butterfly* newButterfly = createInitialIndexedStorage(vm, length, sizeof(EncodedTiValue));
...
ContiguousTiValues JSObject::createInitialInt32(VM& vm, unsigned length)
{
    DeferGC deferGC(vm.heap);
    Butterfly* newButterfly = createInitialIndexedStorage(vm, length, sizeof(EncodedTiValue));
...
ContiguousTiValues JSObject::createInitialContiguous(VM& vm, unsigned length)
{
    DeferGC deferGC(vm.heap);
    Butterfly* newButterfly = createInitialIndexedStorage(vm, length, sizeof(EncodedTiValue));
...
ContiguousTiValues JSObject::convertUndecidedToInt32(VM& vm)
{
...
ContiguousTiValues JSObject::convertUndecidedToContiguous(VM& vm)
{
    ASSERT(hasUndecided(structure()->indexingType()));
...
ArrayStorage* JSObject::constructConvertedArrayStorageWithoutCopyingElements(VM& vm, unsigned neededLength)
{
    ...
    memcpy(
        ...
        propertySize * sizeof(EncodedTiValue));
...
ContiguousTiValues JSObject::convertInt32ToContiguous(VM& vm)
{
    ASSERT(hasInt32(structure()->indexingType()));
...
ArrayStorage* JSObject::convertInt32ToArrayStorage(VM& vm, NonPropertyTransition transition, unsigned neededLength)
{
    ...
    for (unsigned i = m_butterfly->publicLength(); i--;) {
        TiValue v = m_butterfly->contiguous()[i].get();
        if (!v)
            continue;
...
ContiguousTiValues JSObject::genericConvertDoubleToContiguous(VM& vm)
{
    ...
    for (unsigned i = m_butterfly->vectorLength(); i--;) {
        ...
        case EncodeValueAsDouble:
            v = TiValue(TiValue::EncodeAsDouble, value);
            break;
...
ContiguousTiValues JSObject::convertDoubleToContiguous(VM& vm)
{
    return genericConvertDoubleToContiguous<EncodeValueAsDouble>(vm);
}

ContiguousTiValues JSObject::rageConvertDoubleToContiguous(VM& vm)
{
...
ArrayStorage* JSObject::convertDoubleToArrayStorage(VM& vm, NonPropertyTransition transition, unsigned neededLength)
{
    ...
    for (unsigned i = m_butterfly->publicLength(); i--;) {
        ...
        if (value != value)
            continue;
        newStorage->m_vector[i].setWithoutWriteBarrier(TiValue(TiValue::EncodeAsDouble, value));
        newStorage->m_numValuesInVector++;
    }
...
void JSObject::convertUndecidedForValue(VM& vm, TiValue value)
{
    if (value.isInt32()) {
...
void JSObject::createInitialForValueAndSet(VM& vm, unsigned index, TiValue value)
{
    if (value.isInt32()) {
...
void JSObject::convertInt32ForValue(VM& vm, TiValue value)
{
...
void JSObject::setIndexQuicklyToUndecided(VM& vm, unsigned index, TiValue value)
{
...
void JSObject::convertInt32ToDoubleOrContiguousWhilePerformingSetIndex(VM& vm, unsigned index, TiValue value)
{
    ASSERT(!value.isInt32());
...
void JSObject::convertDoubleToContiguousWhilePerformingSetIndex(VM& vm, unsigned index, TiValue value)
{
...
ContiguousTiValues JSObject::ensureInt32Slow(VM& vm)
{
...
ContiguousTiValues JSObject::ensureContiguousSlow(VM& vm, DoubleToContiguousMode mode)
{
    ASSERT(inherits(info()));
...
ContiguousTiValues JSObject::ensureContiguousSlow(VM& vm)
{
    return ensureContiguousSlow(vm, EncodeValueAsDouble);
}

ContiguousTiValues JSObject::rageEnsureContiguousSlow(VM& vm)
{
...
void JSObject::setPrototype(VM& vm, TiValue prototype)
{
    ASSERT(prototype);
...
bool JSObject::setPrototypeWithCycleCheck(ExecState* exec, TiValue prototype)
{
    ASSERT(methodTable()->toThis(this, exec, NotStrictMode) == this);
    TiValue nextPrototype = prototype;
...
void JSObject::putDirectAccessor(ExecState* exec, PropertyName propertyName, TiValue value, unsigned attributes)
{
...
void JSObject::putDirectNonIndexAccessor(VM& vm, PropertyName propertyName, TiValue value, unsigned attributes)
{
...
static ALWAYS_INLINE TiValue callDefaultValueFunction(ExecState* exec, const JSObject* object, PropertyName propertyName)
{
    ...
    TiValue result = call(exec, function, callType, callData, const_cast<JSObject*>(object), exec->emptyList());
...
bool JSObject::getPrimitiveNumber(ExecState* exec, double& number, TiValue& result) const
{
...
// ECMA 8.6.2.6
TiValue JSObject::defaultValue(const JSObject* object, ExecState* exec, PreferredPrimitiveType hint)
{
    // Must call toString first for Date objects.
    if ((hint == PreferString) || (hint != PreferNumber && object->prototype() == exec->lexicalGlobalObject()->datePrototype())) {
        TiValue value = callDefaultValueFunction(exec, object, exec->propertyNames().toString);
        if (value)
            return value;
        ...
        TiValue value = callDefaultValueFunction(exec, object, exec->propertyNames().valueOf);
        if (value)
            return value;
...
bool JSObject::hasInstance(ExecState* exec, TiValue value)
{
...
bool JSObject::defaultHasInstance(ExecState* exec, TiValue value, TiValue proto)
{
    if (!value.isObject())
        return false;
...
TiValue JSObject::toThis(JSCell* cell, ExecState*, ECMAMode)
{
...
NEVER_INLINE void JSObject::fillGetterPropertySlot(PropertySlot& slot, TiValue getterSetter, unsigned attributes, PropertyOffset offset)
{
...
bool JSObject::attemptToInterceptPutByIndexOnHoleForPrototype(ExecState* exec, TiValue thisValue, unsigned i, TiValue value, bool shouldThrow)
{
    for (JSObject* current = this; ;) {
        ...
        TiValue prototypeValue = current->prototype();
...
bool JSObject::attemptToInterceptPutByIndexOnHole(ExecState* exec, unsigned i, TiValue value, bool shouldThrow)
{
    TiValue prototypeValue = prototype();
    if (prototypeValue.isNull())
        return false;
...
void JSObject::putByIndexBeyondVectorLengthWithoutAttributes(ExecState* exec, unsigned i, TiValue value)
{
...
void JSObject::putByIndexBeyondVectorLengthWithArrayStorage(ExecState* exec, unsigned i, TiValue value, bool shouldThrow, ArrayStorage* storage)
{
    VM& vm = exec->vm();
...
void JSObject::putByIndexBeyondVectorLength(ExecState* exec, unsigned i, TiValue value, bool shouldThrow)
{
    VM& vm = exec->vm();
...
bool JSObject::putDirectIndexBeyondVectorLengthWithArrayStorage(ExecState* exec, unsigned i, TiValue value, unsigned attributes, PutDirectIndexMode mode, ArrayStorage* storage)
{
    VM& vm = exec->vm();
...
bool JSObject::putDirectIndexBeyondVectorLength(ExecState* exec, unsigned i, TiValue value, unsigned attributes, PutDirectIndexMode mode)
{
    VM& vm = exec->vm();
...
void JSObject::ensureLengthSlow(VM& vm, unsigned length)
{
    ...
    m_butterfly.set(vm, this, m_butterfly->growArrayRight(
        vm, this, structure(), structure()->outOfLineCapacity(), true,
        oldVectorLength * sizeof(EncodedTiValue),
        newVectorLength * sizeof(EncodedTiValue)));
...
void JSObject::putDirectMayBeIndex(ExecState* exec, PropertyName propertyName, TiValue value)
{
...
bool JSObject::defineOwnNonIndexProperty(ExecState* exec, PropertyName propertyName, const PropertyDescriptor& descriptor, bool throwException)
{
    ...
    TiValue accessor = getDirect(exec->vm(), propertyName);
    if (!accessor)
        return false;
