...
static EncodedTiValue JSC_HOST_CALL arrayConstructorIsArray(ExecState*);
...
JSObject* constructArrayWithSizeQuirk(ExecState* exec, ArrayAllocationProfile* profile, JSGlobalObject* globalObject, TiValue length)
{
    if (!length.isNumber())
...
static EncodedTiValue JSC_HOST_CALL constructWithArrayConstructor(ExecState* exec)
{
    ArgList args(exec);
...
static EncodedTiValue JSC_HOST_CALL callArrayConstructor(ExecState* exec)
{
    ArgList args(exec);
    return TiValue::encode(constructArrayWithSizeQuirk(exec, args));
}
...
EncodedTiValue JSC_HOST_CALL arrayConstructorIsArray(ExecState* exec)
{
    return TiValue::encode(jsBoolean(exec->argument(0).inherits(JSArray::info())));
}
