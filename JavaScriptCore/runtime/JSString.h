...
namespace TI {
    ...
    public:
        ...
        TiValue toPrimitive(ExecState*, PreferredPrimitiveType) const;
    ...
    private:
        ...
        friend TiValue jsString(ExecState*, JSString*, JSString*);
    ...
    private:
        friend TiValue jsStringFromRegisterArray(ExecState*, Register*, unsigned);
        friend TiValue jsStringFromArguments(ExecState*, TiValue);
    ...
    JSString* asString(TiValue);

    inline JSString* asString(TiValue value)
    {
    ...
    inline bool isJSString(TiValue v) { return v.isCell() && v.asCell()->classInfo() == JSString::info(); }

    // --- TiValue inlines ----------------------------
        
    inline bool TiValue::toBoolean(ExecState* exec) const
    {
        if (isInt32())
            return asInt32();
        if (isDouble())
    ...
    inline JSString* TiValue::toString(ExecState* exec) const
    {
        if (isString())
    ...
    inline String TiValue::toWTFString(ExecState* exec) const
    {
        if (isString())
    ...
    ALWAYS_INLINE String inlineTiValueNotStringtoString(const TiValue& value, ExecState* exec)
    {
        VM& vm = exec->vm();
        if (value.isInt32())
    ...
    ALWAYS_INLINE String TiValue::toWTFStringInline(ExecState* exec) const
    {
        if (isString())
            return static_cast<JSString*>(asCell())->value(exec);
