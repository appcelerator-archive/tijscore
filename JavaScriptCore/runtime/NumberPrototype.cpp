...
using namespace WTI::double_conversion;

// To avoid conflict with WTI::StringBuilder.
typedef WTI::double_conversion::StringBuilder DoubleConversionStringBuilder;

namespace TI {

static EncodedTiValue JSC_HOST_CALL numberProtoFuncToString(ExecState*);
static EncodedTiValue JSC_HOST_CALL numberProtoFuncToLocaleString(ExecState*);
static EncodedTiValue JSC_HOST_CALL numberProtoFuncValueOf(ExecState*);
static EncodedTiValue JSC_HOST_CALL numberProtoFuncToFixed(ExecState*);
static EncodedTiValue JSC_HOST_CALL numberProtoFuncToExponential(ExecState*);
static EncodedTiValue JSC_HOST_CALL numberProtoFuncToPrecision(ExecState*);
...
static ALWAYS_INLINE bool getIntegerArgumentInRange(ExecState* exec, int low, int high, int& result, bool& isUndefined)
{
    ...
    TiValue argument0 = exec->argument(0);
...
EncodedTiValue JSC_HOST_CALL numberProtoFuncToExponential(ExecState* exec)
{
    double x;
    ...
    if (!std::isfinite(x))
        return TiValue::encode(jsString(exec, String::numberToStringECMAScript(x)));

    // Round if the argument is not undefined, always format as exponential.
    char buffer[WTI::NumberToStringBufferLength];
    DoubleConversionStringBuilder builder(buffer, WTI::NumberToStringBufferLength);
    ...
    return TiValue::encode(jsString(exec, String(builder.Finalize())));
}
...
EncodedTiValue JSC_HOST_CALL numberProtoFuncToFixed(ExecState* exec)
{
    double x;
    if (!toThisNumber(exec->hostThisValue(), x))
        return throwVMTypeError(exec);
    ...
    return TiValue::encode(jsString(exec, String(numberToFixedWidthString(x, decimalPlaces, buffer))));
}
...
EncodedTiValue JSC_HOST_CALL numberProtoFuncToPrecision(ExecState* exec)
{
    double x;
    if (!toThisNumber(exec->hostThisValue(), x))
        return throwVMTypeError(exec);
    ...
    NumberToStringBuffer buffer;
    return TiValue::encode(jsString(exec, String(numberToFixedPrecisionString(x, significantFigures, buffer))));
}
...
static inline EncodedTiValue integerValueToString(ExecState* exec, int32_t radix, int32_t value)
{
    ...
    if (static_cast<unsigned>(value) < static_cast<unsigned>(radix)) {
        ...
        ASSERT(value >= 0);
        VM* vm = &exec->vm();
        return TiValue::encode(vm->smallStrings.singleCharacterString(radixDigits[value]));
    }

    if (radix == 10) {
        VM* vm = &exec->vm();
        return TiValue::encode(jsString(vm, vm->numericStrings.add(value)));
    }

    return TiValue::encode(jsString(exec, toStringWithRadix(value, radix)));

}

EncodedTiValue JSC_HOST_CALL numberProtoFuncToString(ExecState* exec)
{
    double doubleValue;
    ...
    if (radix == 10) {
        VM* vm = &exec->vm();
        return TiValue::encode(jsString(vm, vm->numericStrings.add(doubleValue)));
    }

    if (!std::isfinite(doubleValue))
        return TiValue::encode(jsString(exec, String::numberToStringECMAScript(doubleValue)));

    RadixBuffer s;
    return TiValue::encode(jsString(exec, toStringWithRadix(s, doubleValue, radix)));
}

EncodedTiValue JSC_HOST_CALL numberProtoFuncToLocaleString(ExecState* exec)
{
    double x;
    if (!toThisNumber(exec->hostThisValue(), x))
        return throwVMTypeError(exec);

    return TiValue::encode(jsNumber(x).toString(exec));
}

EncodedTiValue JSC_HOST_CALL numberProtoFuncValueOf(ExecState* exec)
{
    double x;
    if (!toThisNumber(exec->hostThisValue(), x))
        return throwVMTypeError(exec);
    return TiValue::encode(jsNumber(x));
}
