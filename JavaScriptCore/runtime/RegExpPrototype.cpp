...
static EncodedTiValue JSC_HOST_CALL regExpProtoFuncTest(ExecState*);
static EncodedTiValue JSC_HOST_CALL regExpProtoFuncExec(ExecState*);
static EncodedTiValue JSC_HOST_CALL regExpProtoFuncCompile(ExecState*);
static EncodedTiValue JSC_HOST_CALL regExpProtoFuncToString(ExecState*);
...
EncodedTiValue JSC_HOST_CALL regExpProtoFuncTest(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    if (!thisValue.inherits(RegExpObject::info()))
        return throwVMTypeError(exec);
    return TiValue::encode(jsBoolean(asRegExpObject(thisValue)->test(exec, exec->argument(0).toString(exec))));
}

EncodedTiValue JSC_HOST_CALL regExpProtoFuncExec(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    if (!thisValue.inherits(RegExpObject::info()))
        return throwVMTypeError(exec);
    return TiValue::encode(asRegExpObject(thisValue)->exec(exec, exec->argument(0).toString(exec)));
}

EncodedTiValue JSC_HOST_CALL regExpProtoFuncCompile(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    if (!thisValue.inherits(RegExpObject::info()))
        return throwVMTypeError(exec);
...
EncodedTiValue JSC_HOST_CALL regExpProtoFuncToString(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    if (!thisValue.inherits(RegExpObject::info()))
        return throwVMTypeError(exec);
    ...
    return TiValue::encode(jsMakeNontrivialString(exec, "/", source, postfix));
}
