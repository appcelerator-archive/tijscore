...
void JSCell::put(JSCell* cell, ExecState* exec, PropertyName identifier, TiValue value, PutPropertySlot& slot)
{
    if (cell->isString()) {
        TiValue(cell).putToPrimitive(exec, identifier, value, slot);
        return;
    }
...
void JSCell::putByIndex(JSCell* cell, ExecState* exec, unsigned identifier, TiValue value, bool shouldThrow)
{
    if (cell->isString()) {
        PutPropertySlot slot(cell, shouldThrow);
        TiValue(cell).putToPrimitive(exec, Identifier::from(exec, identifier), value, slot);
        return;
    }
    JSObject* thisObject = cell->toObject(exec, exec->lexicalGlobalObject());
...
TiValue JSCell::toThis(JSCell* cell, ExecState* exec, ECMAMode ecmaMode)
{
...
TiValue JSCell::toPrimitive(ExecState* exec, PreferredPrimitiveType preferredType) const
{
    if (isString())
...
bool JSCell::getPrimitiveNumber(ExecState* exec, double& number, TiValue& value) const
{
    if (isString())
...
TiValue JSCell::defaultValue(const JSObject*, ExecState*, PreferredPrimitiveType)
{
...
bool JSCell::customHasInstance(JSObject*, ExecState*, TiValue)
{
    RELEASE_ASSERT_NOT_REACHED();
    return false;
}
