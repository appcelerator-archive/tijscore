...
static EncodedTiValue regExpObjectGlobal(ExecState*, EncodedTiValue, EncodedTiValue, PropertyName);
static EncodedTiValue regExpObjectIgnoreCase(ExecState*, EncodedTiValue, EncodedTiValue, PropertyName);
static EncodedTiValue regExpObjectMultiline(ExecState*, EncodedTiValue, EncodedTiValue, PropertyName);
static EncodedTiValue regExpObjectSource(ExecState*, EncodedTiValue, EncodedTiValue, PropertyName);
...
static inline RegExpObject* asRegExpObject(EncodedTiValue value)
{
    return jsCast<RegExpObject*>(TiValue::decode(value));
}

EncodedTiValue regExpObjectGlobal(ExecState*, EncodedTiValue slotBase, EncodedTiValue, PropertyName)
{
    return TiValue::encode(jsBoolean(asRegExpObject(slotBase)->regExp()->global()));
}

EncodedTiValue regExpObjectIgnoreCase(ExecState*, EncodedTiValue slotBase, EncodedTiValue, PropertyName)
{
    return TiValue::encode(jsBoolean(asRegExpObject(slotBase)->regExp()->ignoreCase()));
}
 
EncodedTiValue regExpObjectMultiline(ExecState*, EncodedTiValue slotBase, EncodedTiValue, PropertyName)
{            
    return TiValue::encode(jsBoolean(asRegExpObject(slotBase)->regExp()->multiline()));
}
...
template <typename CharacterType>
static inline TiValue regExpObjectSourceInternal(ExecState* exec, String pattern, const CharacterType* characters, unsigned length)
{
...
EncodedTiValue regExpObjectSource(ExecState* exec, EncodedTiValue slotBase, EncodedTiValue, PropertyName)
{
    String pattern = asRegExpObject(slotBase)->regExp()->pattern();
    if (pattern.is8Bit())
        return TiValue::encode(regExpObjectSourceInternal(exec, pattern, pattern.characters8(), pattern.length()));
    return TiValue::encode(regExpObjectSourceInternal(exec, pattern, pattern.characters16(), pattern.length()));
}

void RegExpObject::put(JSCell* cell, ExecState* exec, PropertyName propertyName, TiValue value, PutPropertySlot& slot)
{
    if (propertyName == exec->propertyNames().lastIndex) {
...
TiValue RegExpObject::exec(ExecState* exec, JSString* string)
{
