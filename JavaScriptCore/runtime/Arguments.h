...
private:
    ...
    bool trySetArgument(VM&, size_t argument, TiValue);
    TiValue tryGetArgument(size_t argument);
...
inline bool Arguments::trySetArgument(VM& vm, size_t argument, TiValue value)
{
    if (!isArgument(argument))
        return false;
...
inline TiValue Arguments::tryGetArgument(size_t argument)
{
    if (!isArgument(argument))
        return TiValue();
