...
static EncodedTiValue JSC_HOST_CALL arrayProtoFuncToString(ExecState*);
static EncodedTiValue JSC_HOST_CALL arrayProtoFuncToLocaleString(ExecState*);
static EncodedTiValue JSC_HOST_CALL arrayProtoFuncConcat(ExecState*);
static EncodedTiValue JSC_HOST_CALL arrayProtoFuncJoin(ExecState*);
static EncodedTiValue JSC_HOST_CALL arrayProtoFuncPop(ExecState*);
static EncodedTiValue JSC_HOST_CALL arrayProtoFuncPush(ExecState*);
static EncodedTiValue JSC_HOST_CALL arrayProtoFuncReverse(ExecState*);
static EncodedTiValue JSC_HOST_CALL arrayProtoFuncShift(ExecState*);
static EncodedTiValue JSC_HOST_CALL arrayProtoFuncSlice(ExecState*);
static EncodedTiValue JSC_HOST_CALL arrayProtoFuncSort(ExecState*);
static EncodedTiValue JSC_HOST_CALL arrayProtoFuncSplice(ExecState*);
static EncodedTiValue JSC_HOST_CALL arrayProtoFuncUnShift(ExecState*);
static EncodedTiValue JSC_HOST_CALL arrayProtoFuncEvery(ExecState*);
static EncodedTiValue JSC_HOST_CALL arrayProtoFuncForEach(ExecState*);
static EncodedTiValue JSC_HOST_CALL arrayProtoFuncSome(ExecState*);
static EncodedTiValue JSC_HOST_CALL arrayProtoFuncIndexOf(ExecState*);
static EncodedTiValue JSC_HOST_CALL arrayProtoFuncFilter(ExecState*);
static EncodedTiValue JSC_HOST_CALL arrayProtoFuncMap(ExecState*);
static EncodedTiValue JSC_HOST_CALL arrayProtoFuncReduce(ExecState*);
static EncodedTiValue JSC_HOST_CALL arrayProtoFuncReduceRight(ExecState*);
static EncodedTiValue JSC_HOST_CALL arrayProtoFuncLastIndexOf(ExecState*);
static EncodedTiValue JSC_HOST_CALL arrayProtoFuncValues(ExecState*);
static EncodedTiValue JSC_HOST_CALL arrayProtoFuncKeys(ExecState*);
static EncodedTiValue JSC_HOST_CALL arrayProtoFuncEntries(ExecState*);
...
// Helper function
static TiValue getProperty(ExecState* exec, JSObject* obj, unsigned index)
{
...
static void putProperty(ExecState* exec, JSObject* obj, PropertyName propertyName, TiValue value)
{
...
void shift(ExecState* exec, JSObject* thisObj, unsigned header, unsigned currentCount, unsigned resultCount, unsigned length)
{
    ...
    for (unsigned k = header; k < length - currentCount; ++k) {
        ...
        if (thisObj->getPropertySlot(exec, from, slot)) {
            TiValue value = slot.getValue(exec, from);
...
EncodedTiValue JSC_HOST_CALL arrayProtoFuncToString(ExecState* exec)
{
    ...
    TiValue function = TiValue(thisObject).get(exec, exec->propertyNames().join);

    // 3. If IsCallable(func) is false, then let func be the standard built-in method Object.prototype.toString (15.2.4.2).
    if (!function.isCell())
        return TiValue::encode(jsMakeNontrivialString(exec, "[object ", thisObject->methodTable()->className(thisObject), "]"));
    CallData callData;
    ...
    if (!isJSArray(thisObject) || callType != CallTypeHost || callData.native.function != arrayProtoFuncJoin)
        return TiValue::encode(call(exec, function, callType, callData, thisObject, exec->emptyList()));
    ...
    return TiValue::encode(stringJoiner.join(exec));
}

EncodedTiValue JSC_HOST_CALL arrayProtoFuncToLocaleString(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue().toThis(exec, StrictMode);
...
EncodedTiValue JSC_HOST_CALL arrayProtoFuncJoin(ExecState* exec)
{
    ...
    if (isJSArray(thisObj)) {
        ...
        for (; k < length; k++) {
            ...
            TiValue element = array->getIndexQuickly(k);
            if (!element.isUndefinedOrNull())
...
EncodedTiValue JSC_HOST_CALL arrayProtoFuncConcat(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue().toThis(exec, StrictMode);
    JSArray* arr = constructEmptyArray(exec, 0);
    unsigned n = 0;
    TiValue curArg = thisValue.toObject(exec);
    if (exec->hadException())
        return TiValue::encode(jsUndefined());
    size_t i = 0;
    ...
    while (1) {
        if (curArg.inherits(JSArray::info())) {
            ...
            for (unsigned k = 0; k < length; ++k) {
                TiValue v = getProperty(exec, curObject, k);
                if (exec->hadException())
                    return TiValue::encode(jsUndefined());
                if (v)
...
EncodedTiValue JSC_HOST_CALL arrayProtoFuncPop(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue().toThis(exec, StrictMode);

    if (isJSArray(thisValue))
        return TiValue::encode(asArray(thisValue)->pop(exec));
...
EncodedTiValue JSC_HOST_CALL arrayProtoFuncPush(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue().toThis(exec, StrictMode);
    ...
    for (unsigned n = 0; n < exec->argumentCount(); n++) {
        ...
        if (length + n >= length)
            ...
            Identifier propertyName(exec, TiValue(static_cast<int64_t>(length) + static_cast<int64_t>(n)).toWTFString(exec));
    ...
    TiValue newLength(static_cast<int64_t>(length) + static_cast<int64_t>(exec->argumentCount()));
    putProperty(exec, thisObj, exec->propertyNames().length, newLength);
    return TiValue::encode(newLength);
}

EncodedTiValue JSC_HOST_CALL arrayProtoFuncReverse(ExecState* exec)
{
    JSObject* thisObj = exec->hostThisValue().toThis(exec, StrictMode).toObject(exec);
    unsigned length = thisObj->get(exec, exec->propertyNames().length).toUInt32(exec);
    if (exec->hadException())
...
EncodedTiValue JSC_HOST_CALL arrayProtoFuncShift(ExecState* exec)
{
    JSObject* thisObj = exec->hostThisValue().toThis(exec, StrictMode).toObject(exec);
    unsigned length = thisObj->get(exec, exec->propertyNames().length).toUInt32(exec);
    if (exec->hadException())
...
EncodedTiValue JSC_HOST_CALL arrayProtoFuncSlice(ExecState* exec)
{
    ...
    for (unsigned k = begin; k < end; k++, n++) {
        TiValue v = getProperty(exec, thisObj, k);
        if (exec->hadException())
            return TiValue::encode(jsUndefined());
        if (v)
...
inline TiValue getOrHole(JSObject* obj, ExecState* exec, unsigned propertyName)
{
    PropertySlot slot(obj);
...
static bool attemptFastSort(ExecState* exec, JSObject* thisObj, TiValue function, CallData& callData, CallType& callType)
{
...
static bool performSlowSort(ExecState* exec, JSObject* thisObj, unsigned length, TiValue function, CallData& callData, CallType& callType)
{
    // "Min" sort. Not the fastest, but definitely less code than heapsort
    // or quicksort, and much less swapping than bubblesort/insertionsort.
    for (unsigned i = 0; i < length - 1; ++i) {
        TiValue iObj = getOrHole(thisObj, exec, i);
        if (exec->hadException())
            return false;
        unsigned themin = i;
        TiValue minObj = iObj;
        for (unsigned j = i + 1; j < length; ++j) {
            TiValue jObj = getOrHole(thisObj, exec, j);
            if (exec->hadException())
                return false;
...
EncodedTiValue JSC_HOST_CALL arrayProtoFuncSort(ExecState* exec)
{
    JSObject* thisObj = exec->hostThisValue().toThis(exec, StrictMode).toObject(exec);
    unsigned length = thisObj->get(exec, exec->propertyNames().length).toUInt32(exec);
    ...
    if (length < 1000)
        return performSlowSort(exec, thisObj, length, function, callData, callType) ? TiValue::encode(thisObj) : TiValue::encode(jsUndefined());
    ...
    for (size_t i = 0; i < nameArray.size(); ++i) {
        ...
        TiValue value = getOrHole(thisObj, exec, index);
        if (exec->hadException())
            return TiValue::encode(jsUndefined());
        if (!value)
            continue;
    ...
    for (size_t i = flatArray->length(); i--;) {
        TiValue value = getOrHole(flatArray, exec, i);
...
EncodedTiValue JSC_HOST_CALL arrayProtoFuncSplice(ExecState* exec)
{
    // 15.4.4.12
    ...
    if (!exec->argumentCount())
        return TiValue::encode(constructEmptyArray(exec, 0));
    ...
    for (unsigned k = 0; k < deleteCount; k++) {
        TiValue v = getProperty(exec, thisObj, k + begin);
        if (exec->hadException())
            return TiValue::encode(jsUndefined());
...
EncodedTiValue JSC_HOST_CALL arrayProtoFuncUnShift(ExecState* exec)
{
    // 15.4.4.13
...
EncodedTiValue JSC_HOST_CALL arrayProtoFuncFilter(ExecState* exec)
{
    JSObject* thisObj = exec->hostThisValue().toThis(exec, StrictMode).toObject(exec);
    unsigned length = thisObj->get(exec, exec->propertyNames().length).toUInt32(exec);
    if (exec->hadException())
    ...
    TiValue applyThis = exec->argument(1);
    ...
    if (callType == CallTypeJS && isJSArray(thisObj)) {
        ...
        for (; k < length && !exec->hadException(); ++k) {
            if (!array->canGetIndexQuickly(k))
                break;
            TiValue v = array->getIndexQuickly(k);
...
EncodedTiValue JSC_HOST_CALL arrayProtoFuncMap(ExecState* exec)
{
    JSObject* thisObj = exec->hostThisValue().toThis(exec, StrictMode).toObject(exec);
    unsigned length = thisObj->get(exec, exec->propertyNames().length).toUInt32(exec);
    if (exec->hadException())
...
EncodedTiValue JSC_HOST_CALL arrayProtoFuncEvery(ExecState* exec)
{
    JSObject* thisObj = exec->hostThisValue().toThis(exec, StrictMode).toObject(exec);
    unsigned length = thisObj->get(exec, exec->propertyNames().length).toUInt32(exec);
    if (exec->hadException())
...
EncodedTiValue JSC_HOST_CALL arrayProtoFuncForEach(ExecState* exec)
{
    JSObject* thisObj = exec->hostThisValue().toThis(exec, StrictMode).toObject(exec);
    unsigned length = thisObj->get(exec, exec->propertyNames().length).toUInt32(exec);
    if (exec->hadException())
...
EncodedTiValue JSC_HOST_CALL arrayProtoFuncSome(ExecState* exec)
{
    JSObject* thisObj = exec->hostThisValue().toThis(exec, StrictMode).toObject(exec);
    unsigned length = thisObj->get(exec, exec->propertyNames().length).toUInt32(exec);
    if (exec->hadException())
...
EncodedTiValue JSC_HOST_CALL arrayProtoFuncReduce(ExecState* exec)
{
    JSObject* thisObj = exec->hostThisValue().toThis(exec, StrictMode).toObject(exec);
    unsigned length = thisObj->get(exec, exec->propertyNames().length).toUInt32(exec);
    if (exec->hadException())
...
EncodedTiValue JSC_HOST_CALL arrayProtoFuncReduceRight(ExecState* exec)
{
    JSObject* thisObj = exec->hostThisValue().toThis(exec, StrictMode).toObject(exec);
    unsigned length = thisObj->get(exec, exec->propertyNames().length).toUInt32(exec);
    if (exec->hadException())
...
EncodedTiValue JSC_HOST_CALL arrayProtoFuncIndexOf(ExecState* exec)
{
    // 15.4.4.14
    JSObject* thisObj = exec->hostThisValue().toThis(exec, StrictMode).toObject(exec);
    unsigned length = thisObj->get(exec, exec->propertyNames().length).toUInt32(exec);
...
EncodedTiValue JSC_HOST_CALL arrayProtoFuncLastIndexOf(ExecState* exec)
{
    // 15.4.4.15
    JSObject* thisObj = exec->hostThisValue().toThis(exec, StrictMode).toObject(exec);
    unsigned length = thisObj->get(exec, exec->propertyNames().length).toUInt32(exec);
    ...
    if (exec->argumentCount() >= 2) {
        TiValue fromValue = exec->uncheckedArgument(1);
...
EncodedTiValue JSC_HOST_CALL arrayProtoFuncValues(ExecState* exec)
{
    JSObject* thisObj = exec->hostThisValue().toThis(exec, StrictMode).toObject(exec);
    return TiValue::encode(JSArrayIterator::create(exec, exec->callee()->globalObject()->arrayIteratorStructure(), ArrayIterateValue, thisObj));
}

EncodedTiValue JSC_HOST_CALL arrayProtoFuncEntries(ExecState* exec)
{
    JSObject* thisObj = exec->hostThisValue().toThis(exec, StrictMode).toObject(exec);
    return TiValue::encode(JSArrayIterator::create(exec, exec->callee()->globalObject()->arrayIteratorStructure(), ArrayIterateKeyValue, thisObj));
}
    
EncodedTiValue JSC_HOST_CALL arrayProtoFuncKeys(ExecState* exec)
{
    JSObject* thisObj = exec->hostThisValue().toThis(exec, StrictMode).toObject(exec);
    return TiValue::encode(JSArrayIterator::create(exec, exec->callee()->globalObject()->arrayIteratorStructure(), ArrayIterateKey, thisObj));
}
