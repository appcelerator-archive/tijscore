...
bool checkSyntax(ExecState* exec, const SourceCode& source, TiValue* returnedException)
{
...
TiValue evaluate(ExecState* exec, const SourceCode& source, TiValue thisValue, TiValue* returnedException)
{
    JSLockHolder lock(exec);
    RELEASE_ASSERT(exec->vm().identifierTable == wtfThreadData().currentIdentifierTable());
    ...
    TiValue result = exec->interpreter()->execute(program, exec, thisObj);
