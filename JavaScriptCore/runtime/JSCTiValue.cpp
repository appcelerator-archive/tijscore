...
// ECMA 9.4
double TiValue::toInteger(ExecState* exec) const
{
    if (isInt32())
        return asInt32();
...
double TiValue::toIntegerPreserveNaN(ExecState* exec) const
{
    if (isInt32())
        return asInt32();
...
double TiValue::toNumberSlowCase(ExecState* exec) const
{
...
JSObject* TiValue::toObjectSlowCase(ExecState* exec, JSGlobalObject* globalObject) const
{
    ASSERT(!isCell());
...
TiValue TiValue::toThisSlowCase(ExecState* exec, ECMAMode ecmaMode) const
{
    ASSERT(!isCell());
...
JSObject* TiValue::synthesizePrototype(ExecState* exec) const
{
    if (isCell()) {
        ASSERT(isString());
...
// ECMA 8.7.2
void TiValue::putToPrimitive(ExecState* exec, PropertyName propertyName, TiValue value, PutPropertySlot& slot)
{
    VM& vm = exec->vm();
...
void TiValue::putToPrimitiveByIndex(ExecState* exec, unsigned propertyName, TiValue value, bool shouldThrow)
{
...
void TiValue::dump(PrintStream& out) const
{
    dumpInContext(out, 0);
}

void TiValue::dumpInContext(PrintStream& out, DumpContext* context) const
{
    if (!*this)
        out.print("<TiValue()>");
    else if (isInt32())
...
{
    int64_t bits = WTI::bitwise_cast<int64_t>(number);
...
bool TiValue::isValidCallee()
{
    return asObject(asCell())->globalObject();
}

JSString* TiValue::toStringSlowCase(ExecState* exec) const
{
    VM& vm = exec->vm();
    ASSERT(!isString());
    if (isInt32())
    ...
    ASSERT(isCell());
    TiValue value = asCell()->toPrimitive(exec, PreferString);
...
String TiValue::toWTFStringSlowCase(ExecState* exec) const
{
    return inlineTiValueNotStringtoString(*this, exec);
}
