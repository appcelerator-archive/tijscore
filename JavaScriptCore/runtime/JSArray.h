...
public:
    ...
    void sort(ExecState*);
    void sort(ExecState*, TiValue compareFunction, CallType, const CallData&);
    void sortNumeric(ExecState*, TiValue compareFunction, CallType, const CallData&);

    void push(ExecState*, TiValue);
    TiValue pop(ExecState*);
    ...
    static Structure* createStructure(VM& vm, JSGlobalObject* globalObject, TiValue prototype, IndexingType indexingType)
    {
...
private:
    ...
    void sortNumericVector(ExecState*, TiValue compareFunction, CallType, const CallData&);
    ...
    template<IndexingType indexingType>
    void sortVector(ExecState*, TiValue compareFunction, CallType, const CallData&);
...
inline Butterfly* createContiguousArrayButterfly(VM& vm, JSCell* intendedOwner, unsigned length, unsigned& vectorLength)
{
    ...
    Butterfly* result = Butterfly::create(
        vm, intendedOwner, 0, 0, true, header, vectorLength * sizeof(EncodedTiValue));
    return result;
}
...
inline JSArray* JSArray::tryCreateUninitialized(VM& vm, Structure* structure, unsigned initialLength)
{
    ...
    if (LIKELY(!hasArrayStorage(structure->indexingType()))) {
        ...
        void* temp;
        if (!vm.heap.tryAllocateStorage(0, Butterfly::totalSize(0, 0, true, vectorLength * sizeof(EncodedTiValue)), &temp))
            return 0;
...
inline JSArray* asArray(TiValue value)
{
    return asArray(value.asCell());
}

inline bool isJSArray(JSCell* cell) { return cell->classInfo() == JSArray::info(); }
inline bool isJSArray(TiValue v) { return v.isCell() && isJSArray(v.asCell()); }
...
inline JSArray* constructArray(ExecState* exec, Structure* arrayStructure, const TiValue* values, unsigned length)
{
    VM& vm = exec->vm();
    JSArray* array = JSArray::tryCreateUninitialized(vm, arrayStructure, length);
...
inline JSArray* constructArrayNegativeIndexed(ExecState* exec, Structure* arrayStructure, const TiValue* values, unsigned length)
{
    VM& vm = exec->vm();
    JSArray* array = JSArray::tryCreateUninitialized(vm, arrayStructure, length);
