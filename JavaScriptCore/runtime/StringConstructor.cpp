...
static EncodedTiValue JSC_HOST_CALL stringFromCharCode(ExecState*);
...
static NEVER_INLINE TiValue stringFromCharCodeSlowCase(ExecState* exec)
{
...
static EncodedTiValue JSC_HOST_CALL stringFromCharCode(ExecState* exec)
{
    if (LIKELY(exec->argumentCount() == 1))
        return TiValue::encode(jsSingleCharacterString(exec, exec->uncheckedArgument(0).toUInt32(exec)));
...
static EncodedTiValue JSC_HOST_CALL constructWithStringConstructor(ExecState* exec)
{
    JSGlobalObject* globalObject = asInternalFunction(exec->callee())->globalObject();
    VM& vm = exec->vm();

    if (!exec->argumentCount())
        return TiValue::encode(StringObject::create(vm, globalObject->stringObjectStructure()));
    
    return TiValue::encode(StringObject::create(vm, globalObject->stringObjectStructure(), exec->uncheckedArgument(0).toString(exec)));
}
...
static EncodedTiValue JSC_HOST_CALL callStringConstructor(ExecState* exec)
{
    if (!exec->argumentCount())
        return TiValue::encode(jsEmptyString(exec));
    return TiValue::encode(exec->uncheckedArgument(0).toString(exec));
}
