...
void JSString::visitChildren(JSCell* cell, SlotVisitor& visitor)
{
    ...
    if (!thisObject->isRope()) {
        WTI::StringImpl* ourImpl = thisObject->m_value.impl();
        if (ourImpl->is8Bit())
...
TiValue JSString::toPrimitive(ExecState*, PreferredPrimitiveType) const
{
    return const_cast<JSString*>(this);
}

bool JSString::getPrimitiveNumber(ExecState* exec, double& number, TiValue& result) const
{
    result = this;
...
TiValue JSString::toThis(JSCell* cell, ExecState* exec, ECMAMode ecmaMode)
{
