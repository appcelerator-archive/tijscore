...
static TiValue encode(ExecState* exec, const char* doNotEscape)
{
...
ALWAYS_INLINE
static TiValue decode(ExecState* exec, const CharType* characters, int length, const char* doNotUnescape, bool strict)
{
    JSStringBuilder builder;
    int k = 0;
    UChar u = 0;
...
static TiValue decode(ExecState* exec, const char* doNotUnescape, bool strict)
{
    JSStringBuilder builder;
...
EncodedTiValue JSC_HOST_CALL globalFuncEval(ExecState* exec)
{
    ...
    return TiValue::encode(exec->interpreter()->execute(eval, exec, calleeGlobalObject->globalThis(), calleeGlobalObject));
}

EncodedTiValue JSC_HOST_CALL globalFuncParseInt(ExecState* exec)
{
    TiValue value = exec->argument(0);
    TiValue radixValue = exec->argument(1);
    ...
    if (value.isNumber()) {
        ...
        if (((n < intMaxPlusOne && n >= tenToTheMinus6) || !n) && radixValue.isUndefinedOrNull())
            return TiValue::encode(jsNumber(static_cast<int32_t>(n)));
    }
    ...
    return TiValue::encode(jsNumber(parseInt(s, radixValue.toInt32(exec))));
}

EncodedTiValue JSC_HOST_CALL globalFuncParseFloat(ExecState* exec)
{
    return TiValue::encode(jsNumber(parseFloat(exec->argument(0).toString(exec)->value(exec))));
}

EncodedTiValue JSC_HOST_CALL globalFuncIsNaN(ExecState* exec)
{
    return TiValue::encode(jsBoolean(std::isnan(exec->argument(0).toNumber(exec))));
}

EncodedTiValue JSC_HOST_CALL globalFuncIsFinite(ExecState* exec)
{
    double n = exec->argument(0).toNumber(exec);
    return TiValue::encode(jsBoolean(std::isfinite(n)));
}

EncodedTiValue JSC_HOST_CALL globalFuncDecodeURI(ExecState* exec)
{
...
EncodedTiValue JSC_HOST_CALL globalFuncDecodeURIComponent(ExecState* exec)
{
    return TiValue::encode(decode(exec, "", true));
}

EncodedTiValue JSC_HOST_CALL globalFuncEncodeURI(ExecState* exec)
{
...
EncodedTiValue JSC_HOST_CALL globalFuncEncodeURIComponent(ExecState* exec)
{
...
EncodedTiValue JSC_HOST_CALL globalFuncEscape(ExecState* exec)
{
...
EncodedTiValue JSC_HOST_CALL globalFuncUnescape(ExecState* exec)
{
    ...
    return TiValue::encode(jsString(exec, builder.toString()));
}

EncodedTiValue JSC_HOST_CALL globalFuncThrowTypeError(ExecState* exec)
{
...
public:
    ...
    EncodedTiValue result() { return m_result; }
    ...
    StackVisitor::Status operator()(StackVisitor& visitor)
    {
        ...
        if (m_thisObject->allowsAccessFrom(visitor->callFrame()))
            m_result = TiValue::encode(m_thisObject->prototype());
...
EncodedTiValue JSC_HOST_CALL globalFuncProtoGetter(ExecState* exec)
{
    JSObject* thisObject = jsDynamicCast<JSObject*>(exec->thisValue().toThis(exec, NotStrictMode));

    if (!thisObject)
        return TiValue::encode(exec->thisValue().synthesizePrototype(exec));
...
EncodedTiValue JSC_HOST_CALL globalFuncProtoSetter(ExecState* exec)
{
    TiValue value = exec->argument(0);
