...
EncodedTiValue JSC_HOST_CALL callHostFunctionAsConstructor(ExecState* exec)
{
...
{
    TiValue displayName = getDirect(exec->vm(), exec->vm().propertyNames->displayName);
...
public:
    ...
    TiValue result() const { return m_result; }
    ...
    StackVisitor::Status operator()(StackVisitor& visitor)
    {
        ...
        m_result = TiValue(visitor->createArguments());
...
static TiValue retrieveArguments(ExecState* exec, JSFunction* functionObj)
{
...
EncodedTiValue JSFunction::argumentsGetter(ExecState* exec, EncodedTiValue slotBase, EncodedTiValue, PropertyName)
{
    JSFunction* thisObj = jsCast<JSFunction*>(TiValue::decode(slotBase));
    ASSERT(!thisObj->isHostFunction());

    return TiValue::encode(retrieveArguments(exec, thisObj));
}
...
static TiValue retrieveCallerFunction(ExecState* exec, JSFunction* functionObj)
{
...
EncodedTiValue JSFunction::callerGetter(ExecState* exec, EncodedTiValue slotBase, EncodedTiValue, PropertyName)
{
    JSFunction* thisObj = jsCast<JSFunction*>(TiValue::decode(slotBase));
    ASSERT(!thisObj->isHostFunction());
    TiValue caller = retrieveCallerFunction(exec, thisObj);
    ...
    if (!caller.isObject() || !asObject(caller)->inherits(JSFunction::info()))
        return TiValue::encode(caller);
    JSFunction* function = jsCast<JSFunction*>(caller);
    if (function->isHostFunction() || !function->jsExecutable()->isStrictMode())
        return TiValue::encode(caller);
    return TiValue::encode(throwTypeError(exec, ASCIILiteral("Function.caller used to retrieve strict caller")));
}

EncodedTiValue JSFunction::lengthGetter(ExecState*, EncodedTiValue slotBase, EncodedTiValue, PropertyName)
{
    JSFunction* thisObj = jsCast<JSFunction*>(TiValue::decode(slotBase));
    ASSERT(!thisObj->isHostFunction());
    return TiValue::encode(jsNumber(thisObj->jsExecutable()->parameterCount()));
}

EncodedTiValue JSFunction::nameGetter(ExecState*, EncodedTiValue slotBase, EncodedTiValue, PropertyName)
{
    JSFunction* thisObj = jsCast<JSFunction*>(TiValue::decode(slotBase));
    ASSERT(!thisObj->isHostFunction());
    return TiValue::encode(thisObj->jsExecutable()->nameValue());
}
...
void JSFunction::put(JSCell* cell, ExecState* exec, PropertyName propertyName, TiValue value, PutPropertySlot& slot)
{
    JSFunction* thisObject = jsCast<JSFunction*>(cell);
    if (thisObject->isHostFunction()) {
