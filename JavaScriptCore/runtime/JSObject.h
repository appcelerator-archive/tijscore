...
inline JSCell* getJSFunction(TiValue value)
{
...
inline JSCell* getCallableObject(TiValue value)
{
    if (!value.isCell())
        return 0;
...
public:
    ...
    void setPrototype(VM&, TiValue prototype);
    bool setPrototypeWithCycleCheck(ExecState*, TiValue prototype);
    ...
    JS_EXPORT_PRIVATE static void put(JSCell*, ExecState*, PropertyName, TiValue, PutPropertySlot&);
    JS_EXPORT_PRIVATE static void putByIndex(JSCell*, ExecState*, unsigned propertyName, TiValue, bool shouldThrow);
        
    void putByIndexInline(ExecState* exec, unsigned propertyName, TiValue value, bool shouldThrow)
    {
    ...
    bool putDirectIndex(ExecState* exec, unsigned propertyName, TiValue value, unsigned attributes, PutDirectIndexMode mode)
    {
    ...
    bool putDirectIndex(ExecState* exec, unsigned propertyName, TiValue value)
    {
    ...
    JS_EXPORT_PRIVATE void putDirectMayBeIndex(ExecState*, PropertyName, TiValue);
    ...
    TiValue getIndexQuickly(unsigned i)
    {
        switch (structure()->indexingType()) {
        case ALL_INT32_INDEXING_TYPES:
        ...
        case ALL_DOUBLE_INDEXING_TYPES:
            return TiValue(TiValue::EncodeAsDouble, m_butterfly->contiguousDouble()[i]);
        case ALL_ARRAY_STORAGE_INDEXING_TYPES:
    ...
    TiValue tryGetIndexQuickly(unsigned i)
    {
        switch (structure()->indexingType()) {
        case ALL_BLANK_INDEXING_TYPES:
        case ALL_UNDECIDED_INDEXING_TYPES:
        ...
        case ALL_INT32_INDEXING_TYPES:
            if (i < m_butterfly->publicLength()) {
                TiValue result = m_butterfly->contiguous()[i].get();
        ...
        case ALL_DOUBLE_INDEXING_TYPES: {
            ...
            if (result != result)
                break;
            return TiValue(TiValue::EncodeAsDouble, result);
        }
        case ALL_ARRAY_STORAGE_INDEXING_TYPES:
    ...
    TiValue getDirectIndex(ExecState* exec, unsigned i)
    {
        if (TiValue result = tryGetIndexQuickly(i))
            return result;
    ...
    TiValue getIndex(ExecState* exec, unsigned i)
    {
        if (TiValue result = tryGetIndexQuickly(i))
            return result;
        return get(exec, i);
    ...
    void setIndexQuickly(VM& vm, unsigned i, TiValue v)
    {
        switch (structure()->indexingType()) {
        case ALL_INT32_INDEXING_TYPES: {
    ...
    void initializeIndex(VM& vm, unsigned i, TiValue v)
    {
        switch (structure()->indexingType()) {
        case ALL_UNDECIDED_INDEXING_TYPES: {
    ...
    void putDirect(VM&, PropertyName, TiValue, unsigned attributes = 0);
    void putDirect(VM&, PropertyName, TiValue, PutPropertySlot&);
    void putDirectWithoutTransition(VM&, PropertyName, TiValue, unsigned attributes = 0);
    void putDirectNonIndexAccessor(VM&, PropertyName, TiValue, unsigned attributes);
    void putDirectAccessor(ExecState*, PropertyName, TiValue, unsigned attributes);
    ...
    JS_EXPORT_PRIVATE static TiValue defaultValue(const JSObject*, ExecState*, PreferredPrimitiveType);

    bool hasInstance(ExecState*, TiValue);
    static bool defaultHasInstance(ExecState*, TiValue, TiValue prototypeProperty);
    ...
    bool getPrimitiveNumber(ExecState*, double& number, TiValue&) const;
    JS_EXPORT_PRIVATE double toNumber(ExecState*) const;
    JS_EXPORT_PRIVATE JSString* toString(ExecState*) const;

    JS_EXPORT_PRIVATE static TiValue toThis(JSCell*, ExecState*, ECMAMode);

    bool getPropertySpecificValue(ExecState*, PropertyName, JSCell*& specificFunction) const;

    // This get function only looks at the property map.
    TiValue getDirect(VM& vm, PropertyName propertyName) const
    {
        PropertyOffset offset = structure()->get(vm, propertyName);
        checkOffset(offset, structure()->inlineCapacity());
        return offset != invalidOffset ? getDirect(offset) : TiValue();
    }

    TiValue getDirect(VM& vm, PropertyName propertyName, unsigned& attributes) const
    {
        JSCell* specific;
    ...
    bool putOwnDataProperty(VM&, PropertyName, TiValue, PutPropertySlot&);

    // Fast access to known property offsets.
    TiValue getDirect(PropertyOffset offset) const { return locationForOffset(offset)->get(); }
    void putDirect(VM& vm, PropertyOffset offset, TiValue value) { locationForOffset(offset)->set(vm, this, value); }
    ...
    bool attemptToInterceptPutByIndexOnHoleForPrototype(ExecState*, TiValue thisValue, unsigned propertyName, TiValue, bool shouldThrow);
    ...
    ContiguousTiValues ensureInt32(VM& vm)
    {
    ...
    ContiguousTiValues ensureContiguous(VM& vm)
    {
    ...
    ContiguousTiValues rageEnsureContiguous(VM& vm)
    {
        if (LIKELY(hasContiguous(structure()->indexingType())))
            return m_butterfly->contiguous();
...
protected:
    ...
    ContiguousTiValues createInitialInt32(VM&, unsigned length);
    ContiguousDoubles createInitialDouble(VM&, unsigned length);
    ContiguousTiValues createInitialContiguous(VM&, unsigned length);
        
    void convertUndecidedForValue(VM&, TiValue);
    void createInitialForValueAndSet(VM&, unsigned index, TiValue);
    void convertInt32ForValue(VM&, TiValue);
        
    ArrayStorage* createArrayStorage(VM&, unsigned length, unsigned vectorLength);
    ArrayStorage* createInitialArrayStorage(VM&);
        
    ContiguousTiValues convertUndecidedToInt32(VM&);
    ContiguousDoubles convertUndecidedToDouble(VM&);
    ContiguousTiValues convertUndecidedToContiguous(VM&);
    ...
    ContiguousTiValues convertInt32ToContiguous(VM&);
    ...
    ContiguousTiValues convertDoubleToContiguous(VM&);
    ContiguousTiValues rageConvertDoubleToContiguous(VM&);
    ...
    void putByIndexBeyondVectorLengthWithoutAttributes(ExecState*, unsigned propertyName, TiValue);
    void putByIndexBeyondVectorLengthWithArrayStorage(ExecState*, unsigned propertyName, TiValue, bool shouldThrow, ArrayStorage*);
    ...
    bool attemptToInterceptPutByIndexOnHole(ExecState*, unsigned index, TiValue, bool shouldThrow);
    ...
    // as if it contained TiValues. But it won't always contain TiValues.
    // Make sure you cast this to the appropriate type before using.
    template<IndexingType indexingType>
    ContiguousTiValues indexingData()
    {
        switch (indexingType) {
        case ALL_INT32_INDEXING_TYPES:
        case ALL_DOUBLE_INDEXING_TYPES:
        ...
        default:
            CRASH();
            return ContiguousTiValues();
        }
    }

    ContiguousTiValues currentIndexingData()
    {
        switch (structure()->indexingType()) {
        case ALL_INT32_INDEXING_TYPES:
        case ALL_CONTIGUOUS_INDEXING_TYPES:
    ...
    TiValue getHolyIndexQuickly(unsigned i)
    {
        ASSERT(i < m_butterfly->vectorLength());
        switch (structure()->indexingType()) {
        case ALL_INT32_INDEXING_TYPES:
        ...
        case ALL_DOUBLE_INDEXING_TYPES: {
            double value = m_butterfly->contiguousDouble()[i];
            if (value == value)
                return TiValue(TiValue::EncodeAsDouble, value);
            return TiValue();
        }
        case ALL_ARRAY_STORAGE_INDEXING_TYPES:
            return m_butterfly->arrayStorage()->m_vector[i].get();
...
private:
    ...
    template<PutMode>
    bool putDirectInternal(VM&, PropertyName, TiValue, unsigned attr, PutPropertySlot&, JSCell*);

    bool inlineGetOwnPropertySlot(ExecState*, PropertyName, PropertySlot&);
    JS_EXPORT_PRIVATE void fillGetterPropertySlot(PropertySlot&, TiValue, unsigned, PropertyOffset);
    ...
    void putByIndexBeyondVectorLength(ExecState*, unsigned propertyName, TiValue, bool shouldThrow);
    bool putDirectIndexBeyondVectorLengthWithArrayStorage(ExecState*, unsigned propertyName, TiValue, unsigned attributes, PutDirectIndexMode, ArrayStorage*);
    JS_EXPORT_PRIVATE bool putDirectIndexBeyondVectorLength(ExecState*, unsigned propertyName, TiValue, unsigned attributes, PutDirectIndexMode);
    ...
    JS_EXPORT_PRIVATE void setIndexQuicklyToUndecided(VM&, unsigned index, TiValue);
    JS_EXPORT_PRIVATE void convertInt32ToDoubleOrContiguousWhilePerformingSetIndex(VM&, unsigned index, TiValue);
    JS_EXPORT_PRIVATE void convertDoubleToContiguousWhilePerformingSetIndex(VM&, unsigned index, TiValue);
        
    void ensureLengthSlow(VM&, unsigned length);
        
    ContiguousTiValues ensureInt32Slow(VM&);
    ContiguousDoubles ensureDoubleSlow(VM&);
    ContiguousTiValues ensureContiguousSlow(VM&);
    ContiguousTiValues rageEnsureContiguousSlow(VM&);
    ...
    ContiguousTiValues genericConvertDoubleToContiguous(VM&);
    ContiguousTiValues ensureContiguousSlow(VM&, DoubleToContiguousMode);
...
public:
    ...
    static Structure* createStructure(VM& vm, JSGlobalObject* globalObject, TiValue prototype, unsigned inlineCapacity)
    {
...
inline bool isJSFinalObject(TiValue value)
{
...
inline JSObject* asObject(TiValue value)
{
...
inline TiValue JSObject::prototype() const
{
...
ALWAYS_INLINE bool JSObject::inlineGetOwnPropertySlot(ExecState* exec, PropertyName propertyName, PropertySlot& slot)
{
    ...
    if (LIKELY(isValidOffset(offset))) {
        TiValue value = getDirect(offset);
...
inline TiValue JSObject::get(ExecState* exec, PropertyName propertyName) const
{
    PropertySlot slot(this);
...
inline TiValue JSObject::get(ExecState* exec, unsigned propertyName) const
{
    PropertySlot slot(this);
    if (const_cast<JSObject*>(this)->getPropertySlot(exec, propertyName, slot))
        return slot.getValue(exec, propertyName);
...
inline bool JSObject::putOwnDataProperty(VM& vm, PropertyName propertyName, TiValue value, PutPropertySlot& slot)
{
    ASSERT(value);
    ASSERT(!Heap::heap(value) || Heap::heap(value) == Heap::heap(this));
...
inline void JSObject::putDirect(VM& vm, PropertyName propertyName, TiValue value, unsigned attributes)
{
    ASSERT(!value.isGetterSetter() && !(attributes & Accessor));
...
inline void JSObject::putDirect(VM& vm, PropertyName propertyName, TiValue value, PutPropertySlot& slot)
{
    ASSERT(!value.isGetterSetter());
    putDirectInternal<PutModeDefineOwnProperty>(vm, propertyName, value, 0, slot, getCallableObject(value));
}

inline void JSObject::putDirectWithoutTransition(VM& vm, PropertyName propertyName, TiValue value, unsigned attributes)
{
    DeferGC deferGC(vm.heap);
    ASSERT(!value.isGetterSetter() && !(attributes & Accessor));
...
inline TiValue JSObject::toPrimitive(ExecState* exec, PreferredPrimitiveType preferredType) const
{
...
inline size_t offsetRelativeToPatchedStorage(PropertyOffset offset)
{
    if (isOutOfLineOffset(offset))
        return sizeof(EncodedTiValue) * offsetInButterfly(offset);
    return JSObject::offsetOfInlineStorage() - JSObject::butterflyOffset() + sizeof(EncodedTiValue) * offsetInInlineStorage(offset);
}
...
inline int indexRelativeToBase(PropertyOffset offset)
{
    if (isOutOfLineOffset(offset))
        return offsetInOutOfLineStorage(offset) + Butterfly::indexOfPropertyStorage();
    ASSERT(!(JSObject::offsetOfInlineStorage() % sizeof(EncodedTiValue)));
    return JSObject::offsetOfInlineStorage() / sizeof(EncodedTiValue) + offsetInInlineStorage(offset);
}

inline int offsetRelativeToBase(PropertyOffset offset)
{
    if (isOutOfLineOffset(offset))
        return offsetInOutOfLineStorage(offset) * sizeof(EncodedTiValue) + Butterfly::offsetOfPropertyStorage();
    return JSObject::offsetOfInlineStorage() + offsetInInlineStorage(offset) * sizeof(EncodedTiValue);
}
...
ALWAYS_INLINE TiValue PropertySlot::getValue(ExecState* exec, PropertyName propertyName) const
{
    if (m_propertyType == TypeValue)
        return TiValue::decode(m_data.value);
    if (m_propertyType == TypeCustomIndex)
        return TiValue::decode(m_data.customIndex.getIndexValue(exec, TiValue::encode(slotBase()), TiValue::encode(m_thisValue), m_data.customIndex.index));
    if (m_propertyType == TypeGetter)
        return functionGetter(exec);
    return TiValue::decode(m_data.custom.getValue(exec, TiValue::encode(slotBase()), TiValue::encode(m_thisValue), propertyName));
}

ALWAYS_INLINE TiValue PropertySlot::getValue(ExecState* exec, unsigned propertyName) const
{
    if (m_propertyType == TypeValue)
        return TiValue::decode(m_data.value);
    if (m_propertyType == TypeCustomIndex)
    ...
    if (m_propertyType == TypeGetter)
        return functionGetter(exec);
    return TiValue::decode(m_data.custom.getValue(exec, TiValue::encode(slotBase()), TiValue::encode(m_thisValue), Identifier::from(exec, propertyName)));
}
