...
TiValue jsTypeStringForValue(VM&, JSGlobalObject*, TiValue);
bool jsIsObjectType(CallFrame*, TiValue);
bool jsIsFunctionType(TiValue);

ALWAYS_INLINE TiValue jsString(ExecState* exec, JSString* s1, JSString* s2)
{
    VM& vm = exec->vm();
...
ALWAYS_INLINE TiValue jsString(ExecState* exec, const String& u1, const String& u2, const String& u3)
{
    VM* vm = &exec->vm();
...
ALWAYS_INLINE TiValue jsStringFromRegisterArray(ExecState* exec, Register* strings, unsigned count)
{
    VM* vm = &exec->vm();
    ...
    for (unsigned i = 0; i < count; ++i) {
        TiValue v = strings[-static_cast<int>(i)].jsValue();
...
ALWAYS_INLINE TiValue jsStringFromArguments(ExecState* exec, TiValue thisValue)
{
    VM* vm = &exec->vm();
    JSRopeString::RopeBuilder ropeBuilder(*vm);
...
template<bool leftFirst>
ALWAYS_INLINE bool jsLessEq(CallFrame* callFrame, TiValue v1, TiValue v2)
{
    if (v1.isInt32() && v2.isInt32())
        return v1.asInt32() <= v2.asInt32();
...
inline size_t normalizePrototypeChainForChainAccess(CallFrame* callFrame, TiValue base, TiValue slotBase, const Identifier& propertyName, PropertyOffset& slotOffset)
{
...
inline bool isPrototypeChainNormalized(JSGlobalObject* globalObject, Structure* structure)
{
    for (;;) {
        ...
        TiValue v = structure->prototypeForLookup(globalObject);
        if (v.isNull())
            return true;
