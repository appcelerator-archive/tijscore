...
// ECMA 15.4.5.1
void JSArray::put(JSCell* cell, ExecState* exec, PropertyName propertyName, TiValue value, PutPropertySlot& slot)
{
...
bool JSArray::unshiftCountSlowCase(VM& vm, bool addToFront, unsigned count)
{
    ...
    if (addToFront) {
        ...
        memmove(newButterfly->arrayStorage()->m_vector + count, storage->m_vector, sizeof(TiValue) * usedVectorLength);
        memmove(newButterfly->propertyStorage() - propertySize, butterfly->propertyStorage() - propertySize, sizeof(TiValue) * propertySize + sizeof(IndexingHeader) + ArrayStorage::sizeFor(0));
    } else if ((newAllocBase != butterfly->base(structure())) || (newIndexBias != storage->m_indexBias)) {
        memmove(newButterfly->propertyStorage() - propertySize, butterfly->propertyStorage() - propertySize, sizeof(TiValue) * propertySize + sizeof(IndexingHeader) + ArrayStorage::sizeFor(0));
        memmove(newButterfly->arrayStorage()->m_vector, storage->m_vector, sizeof(TiValue) * usedVectorLength);
...
TiValue JSArray::pop(ExecState* exec)
{
    switch (structure()->indexingType()) {
    case ArrayClass:
        return jsUndefined();
    ...
    case ArrayWithContiguous: {
        ...
        TiValue value = m_butterfly->contiguous()[length].get();
        if (value) {
    ...
    TiValue element = get(exec, index);
...
void JSArray::push(ExecState* exec, TiValue value)
{
    switch (structure()->indexingType()) {
    case ArrayClass: {
...
bool JSArray::shiftCountWithArrayStorage(unsigned startIndex, unsigned count, ArrayStorage* storage)
{
    ...
    if (numElementsBeforeShiftRegion < numElementsAfterShiftRegion) {
        ...
        memmove(
            ...
            sizeof(TiValue) * numElementsAfterShiftRegion);
...
bool JSArray::shiftCountWithAnyIndexingType(ExecState* exec, unsigned startIndex, unsigned count)
{
    ...
    case ArrayWithContiguous: {
        ...
        for (unsigned i = startIndex; i < end; ++i) {
            TiValue v = m_butterfly->contiguous()[i + count].get();
            if (UNLIKELY(!v))
                return shiftCountWithArrayStorage(startIndex, count, ensureArrayStorage(exec->vm()));
        }
...
bool JSArray::unshiftCountWithArrayStorage(ExecState* exec, unsigned startIndex, unsigned count, ArrayStorage* storage)
{
    ...
    if (startIndex) {
        if (moveFront)
            memmove(vector, vector + count, startIndex * sizeof(TiValue));
        else if (length - startIndex)
            memmove(vector + startIndex + count, vector + startIndex, (length - startIndex) * sizeof(TiValue));
    }
...
{
    int32_t ia = static_cast<const TiValue*>(a)->asInt32();
    int32_t ib = static_cast<const TiValue*>(b)->asInt32();
    return ia - ib;
}
...
{
    double da = static_cast<const TiValue*>(a)->asNumber();
    double db = static_cast<const TiValue*>(b)->asNumber();
    return (da > db) - (da < db);
}
...
void JSArray::sortNumericVector(ExecState* exec, TiValue compareFunction, CallType callType, const CallData& callData)
{
    ...
    ContiguousTiValues data = indexingData<indexingType>();
...
void JSArray::sortNumeric(ExecState* exec, TiValue compareFunction, CallType callType, const CallData& callData)
{
...
template <IndexingType> struct ContiguousTypeAccessor {
    ...
    static TiValue getAsValue(ContiguousData<Type> data, size_t i) { return data[i].get(); }
    static void setWithValue(VM& vm, JSArray* thisValue, ContiguousData<Type> data, size_t i, TiValue value)
    {
        data[i].set(vm, thisValue, value);
    }
    static void replaceDataReference(ContiguousData<Type>* outData, ContiguousTiValues inData)
    {
        *outData = inData;
    }
};
...
template <> struct ContiguousTypeAccessor<ArrayWithDouble> {
    typedef double Type;
    static TiValue getAsValue(ContiguousData<Type> data, size_t i) { ASSERT(data[i] == data[i]); return TiValue(TiValue::EncodeAsDouble, data[i]); }
    static void setWithValue(VM&, JSArray*, ContiguousData<Type> data, size_t i, TiValue value)
    {
        data[i] = value.asNumber();
    }
    static NO_RETURN_DUE_TO_CRASH void replaceDataReference(ContiguousData<Type>*, ContiguousTiValues)
    {
...
void JSArray::sortCompactedVector(ExecState* exec, ContiguousData<StorageType> data, unsigned relevantLength)
{
    ...
    for (size_t i = 0; i < relevantLength; i++) {
        TiValue value = ContiguousTypeAccessor<indexingType>::getAsValue(data, i);
...
template<IndexingType indexingType>
void JSArray::sortVector(ExecState* exec, TiValue compareFunction, CallType callType, const CallData& callData)
{
    ASSERT(!inSparseIndexingMode());
    ...
    for (; numDefined < usedVectorLength; ++numDefined) {
        if (numDefined >= m_butterfly->vectorLength())
            break;
        TiValue v = getHolyIndexQuickly(numDefined);
        if (!v || v.isUndefined())
            break;
    ...
    for (unsigned i = numDefined; i < usedVectorLength; ++i) {
        if (i >= m_butterfly->vectorLength())
            break;
        TiValue v = getHolyIndexQuickly(i);
        if (v) {
            if (v.isUndefined())
                ++numUndefined;
            else {
...
void JSArray::sort(ExecState* exec, TiValue compareFunction, CallType callType, const CallData& callData)
{
    ASSERT(!inSparseIndexingMode());
...
void JSArray::fillArgList(ExecState* exec, MarkedArgumentBuffer& args)
{
    ...
    case ArrayWithDouble: {
        ...
        for (; i < m_butterfly->publicLength(); ++i) {
            ...
            if (v != v)
                break;
            args.append(TiValue(TiValue::EncodeAsDouble, v));
        }
        break;
    }
...
void JSArray::copyToArguments(ExecState* exec, CallFrame* callFrame, uint32_t length)
{
    ...
    case ArrayWithDouble: {
        ...
        for (; i < m_butterfly->publicLength(); ++i) {
            ...
            if (v != v)
                break;
            callFrame->setArgument(i, TiValue(TiValue::EncodeAsDouble, v));
        }
        break;
    }
...
void JSArray::compactForSorting(unsigned& numDefined, unsigned& newRelevantLength)
{
    ...
    for (; numDefined < myRelevantLength; ++numDefined) {
        ...
        if (indexingType == ArrayWithInt32) {
            TiValue v = m_butterfly->contiguousInt32()[numDefined].get();
            if (!v)
                break;
            ASSERT(v.isInt32());
            continue;
        ...
        TiValue v = indexingData<indexingType>()[numDefined].get();
        if (!v || v.isUndefined())
            break;
    }
    ...
    for (unsigned i = numDefined; i < myRelevantLength; ++i) {
        ...
        if (indexingType == ArrayWithInt32) {
            TiValue v = m_butterfly->contiguousInt32()[i].get();
            if (!v)
                continue;
            ASSERT(v.isInt32());
            ASSERT(numDefined < m_butterfly->vectorLength());
        ...
        TiValue v = indexingData<indexingType>()[i].get();
        if (v) {
            if (v.isUndefined())
                ++numUndefined;
            else {
