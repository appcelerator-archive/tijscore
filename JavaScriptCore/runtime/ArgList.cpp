...
void MarkedArgumentBuffer::markLists(HeapRootVisitor& heapRootVisitor, ListSet& markSet)
{
    ...
    for (ListSet::iterator it = markSet.begin(); it != end; ++it) {
        ...
        for (int i = 0; i < list->m_size; ++i)
            heapRootVisitor.visit(reinterpret_cast<TiValue*>(&list->slotFor(i)));
    }
}

void MarkedArgumentBuffer::slowAppend(TiValue v)
{
    int newCapacity = m_capacity * 4;
    EncodedTiValue* newBuffer = new EncodedTiValue[newCapacity];
    ...
    for (int i = 0; i < m_size; ++i) {
        Heap* heap = Heap::heap(TiValue::decode(slotFor(i)));
        if (!heap)
            continue;
