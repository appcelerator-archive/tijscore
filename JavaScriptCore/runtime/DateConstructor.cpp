...
static EncodedTiValue JSC_HOST_CALL dateParse(ExecState*);
static EncodedTiValue JSC_HOST_CALL dateNow(ExecState*);
static EncodedTiValue JSC_HOST_CALL dateUTC(ExecState*);
...
JSObject* constructDate(ExecState* exec, JSGlobalObject* globalObject, const ArgList& args)
{
    ...
    if (numArgs == 0) // new Date() ECMA 15.9.3.3
        ...
        if (!std::isfinite(doubleArguments[0])
            ...
            t.setMonth(TI::toInt32(doubleArguments[1]));
            t.setMonthDay((numArgs >= 3) ? TI::toInt32(doubleArguments[2]) : 1);
            t.setHour(TI::toInt32(doubleArguments[3]));
            t.setMinute(TI::toInt32(doubleArguments[4]));
            t.setSecond(TI::toInt32(doubleArguments[5]));
            t.setIsDST(-1);
...
static EncodedTiValue JSC_HOST_CALL constructWithDateConstructor(ExecState* exec)
{
    ArgList args(exec);
...
// ECMA 15.9.2
static EncodedTiValue JSC_HOST_CALL callDate(ExecState* exec)
{
    VM& vm = exec->vm();
    GregorianDateTime ts;
    msToGregorianDateTime(vm, currentTimeMS(), false, ts);
    return TiValue::encode(jsNontrivialString(&vm, formatDateTime(ts, DateTimeFormatDateAndTime, false)));
}
...
static EncodedTiValue JSC_HOST_CALL dateParse(ExecState* exec)
{
    return TiValue::encode(jsNumber(parseDate(exec->vm(), exec->argument(0).toString(exec)->value(exec))));
}
...
static EncodedTiValue JSC_HOST_CALL dateUTC(ExecState* exec) 
{
    double doubleArguments[7] = {
    ...
    t.setYear((year >= 0 && year <= 99) ? (year + 1900) : year);
    t.setMonth(TI::toInt32(doubleArguments[1]));
    t.setMonthDay((n >= 3) ? TI::toInt32(doubleArguments[2]) : 1);
    t.setHour(TI::toInt32(doubleArguments[3]));
    t.setMinute(TI::toInt32(doubleArguments[4]));
    t.setSecond(TI::toInt32(doubleArguments[5]));
    double ms = (n >= 7) ? doubleArguments[6] : 0;
    return TiValue::encode(jsNumber(timeClip(gregorianDateTimeToMS(exec->vm(), t, ms, true))));
}
