...
static EncodedTiValue JSC_HOST_CALL argumentsFuncIterator(ExecState*);
...
void Arguments::copyToArguments(ExecState* exec, CallFrame* callFrame, uint32_t length)
{
    ...
    for (size_t i = 0; i < length; ++i) {
        if (TiValue value = tryGetArgument(i))
...
bool Arguments::getOwnPropertySlotByIndex(JSObject* object, ExecState* exec, unsigned i, PropertySlot& slot)
{
    ...
    if (TiValue value = thisObject->tryGetArgument(i)) {
...
void Arguments::putByIndex(JSCell* cell, ExecState* exec, unsigned i, TiValue value, bool shouldThrow)
{
    Arguments* thisObject = jsCast<Arguments*>(cell);
...
void Arguments::put(JSCell* cell, ExecState* exec, PropertyName propertyName, TiValue value, PutPropertySlot& slot)
{
    Arguments* thisObject = jsCast<Arguments*>(cell);
    unsigned i = propertyName.asIndex();
    if (thisObject->trySetArgument(exec->vm(), i, value))
...
bool Arguments::defineOwnProperty(JSObject* object, ExecState* exec, PropertyName propertyName, const PropertyDescriptor& descriptor, bool shouldThrow)
{
    ...
    if (i < thisObject->m_numArguments) {
        ...
        if (!thisObject->isDeletedArgument(i) && !JSObject::getOwnPropertySlot(thisObject, exec, propertyName, slot)) {
            TiValue value = thisObject->tryGetArgument(i);
            ASSERT(value);
...
EncodedTiValue JSC_HOST_CALL argumentsFuncIterator(ExecState* exec)
{
    ...
    if (!arguments)
        return TiValue::encode(throwTypeError(exec, "Attempted to use Arguments iterator on non-Arguments object"));
    return TiValue::encode(JSArgumentsIterator::create(exec->vm(), exec->callee()->globalObject()->argumentsIteratorStructure(), arguments));
}
