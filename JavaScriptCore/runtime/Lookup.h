...
namespace TI {
    ...
    // ie. typedef TiValue (*GetFunction)(ExecState*, JSObject* baseObject)
    ...
    inline void putEntry(ExecState* exec, const HashEntry* entry, PropertyName propertyName, TiValue value, PutPropertySlot& slot)
    {
        // If this is a function put it as an override property.
        if (entry->attributes() & Function)
            slot.base()->putDirect(exec->vm(), propertyName, value);
        else if (!(entry->attributes() & ReadOnly)) {
            entry->propertyPutter()(exec, TiValue::encode(slot.thisValue()), TiValue::encode(value));
     ...
     */
    inline bool lookupPut(ExecState* exec, PropertyName propertyName, TiValue value, const HashTable& table, PutPropertySlot& slot)
    {
        const HashEntry* entry = table.entry(exec, propertyName);
     ...
     */
    template <class ThisImp, class ParentImp>
    inline void lookupPut(ExecState* exec, PropertyName propertyName, TiValue value, const HashTable& table, ThisImp* thisObj, PutPropertySlot& slot)
    {
