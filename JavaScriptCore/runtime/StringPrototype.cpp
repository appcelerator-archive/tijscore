...
static EncodedTiValue JSC_HOST_CALL stringProtoFuncToString(ExecState*);
static EncodedTiValue JSC_HOST_CALL stringProtoFuncCharAt(ExecState*);
static EncodedTiValue JSC_HOST_CALL stringProtoFuncCharCodeAt(ExecState*);
static EncodedTiValue JSC_HOST_CALL stringProtoFuncConcat(ExecState*);
static EncodedTiValue JSC_HOST_CALL stringProtoFuncIndexOf(ExecState*);
static EncodedTiValue JSC_HOST_CALL stringProtoFuncLastIndexOf(ExecState*);
static EncodedTiValue JSC_HOST_CALL stringProtoFuncMatch(ExecState*);
static EncodedTiValue JSC_HOST_CALL stringProtoFuncReplace(ExecState*);
static EncodedTiValue JSC_HOST_CALL stringProtoFuncSearch(ExecState*);
static EncodedTiValue JSC_HOST_CALL stringProtoFuncSlice(ExecState*);
static EncodedTiValue JSC_HOST_CALL stringProtoFuncSplit(ExecState*);
static EncodedTiValue JSC_HOST_CALL stringProtoFuncSubstr(ExecState*);
static EncodedTiValue JSC_HOST_CALL stringProtoFuncSubstring(ExecState*);
static EncodedTiValue JSC_HOST_CALL stringProtoFuncToLowerCase(ExecState*);
static EncodedTiValue JSC_HOST_CALL stringProtoFuncToUpperCase(ExecState*);
static EncodedTiValue JSC_HOST_CALL stringProtoFuncLocaleCompare(ExecState*);
static EncodedTiValue JSC_HOST_CALL stringProtoFuncBig(ExecState*);
static EncodedTiValue JSC_HOST_CALL stringProtoFuncSmall(ExecState*);
static EncodedTiValue JSC_HOST_CALL stringProtoFuncBlink(ExecState*);
static EncodedTiValue JSC_HOST_CALL stringProtoFuncBold(ExecState*);
static EncodedTiValue JSC_HOST_CALL stringProtoFuncFixed(ExecState*);
static EncodedTiValue JSC_HOST_CALL stringProtoFuncItalics(ExecState*);
static EncodedTiValue JSC_HOST_CALL stringProtoFuncStrike(ExecState*);
static EncodedTiValue JSC_HOST_CALL stringProtoFuncSub(ExecState*);
static EncodedTiValue JSC_HOST_CALL stringProtoFuncSup(ExecState*);
static EncodedTiValue JSC_HOST_CALL stringProtoFuncFontcolor(ExecState*);
static EncodedTiValue JSC_HOST_CALL stringProtoFuncFontsize(ExecState*);
static EncodedTiValue JSC_HOST_CALL stringProtoFuncAnchor(ExecState*);
static EncodedTiValue JSC_HOST_CALL stringProtoFuncLink(ExecState*);
static EncodedTiValue JSC_HOST_CALL stringProtoFuncTrim(ExecState*);
static EncodedTiValue JSC_HOST_CALL stringProtoFuncTrimLeft(ExecState*);
static EncodedTiValue JSC_HOST_CALL stringProtoFuncTrimRight(ExecState*);
...
static inline JSString* jsStringWithReuse(ExecState* exec, TiValue originalValue, const String& string)
{
...
static ALWAYS_INLINE TiValue jsSpliceSubstrings(ExecState* exec, JSString* sourceVal, const String& source, const StringRange* substringRanges, int rangeCount)
{
    if (rangeCount == 1) {
...
static ALWAYS_INLINE TiValue jsSpliceSubstringsWithSeparators(ExecState* exec, JSString* sourceVal, const String& source, const StringRange* substringRanges, int rangeCount, const String* separators, int separatorCount)
{
...
static NEVER_INLINE EncodedTiValue removeUsingRegExpSearch(ExecState* exec, JSString* string, const String& source, RegExp* regExp)
{
    size_t lastIndex = 0;
    ...
    if (!lastIndex)
        return TiValue::encode(string);

    if (static_cast<unsigned>(lastIndex) < sourceLen)
        sourceRanges.append(StringRange(lastIndex, sourceLen - lastIndex));

    return TiValue::encode(jsSpliceSubstrings(exec, string, source, sourceRanges.data(), sourceRanges.size()));
}

static NEVER_INLINE EncodedTiValue replaceUsingRegExpSearch(ExecState* exec, JSString* string, TiValue searchValue)
{
    TiValue replaceValue = exec->argument(1);
    ...
    if (global && callType == CallTypeJS) {
        ...
        if (source.is8Bit()) {
            while (true) {
                ...
                TiValue jsResult = cachedCall.call();
    ...
    return TiValue::encode(jsSpliceSubstringsWithSeparators(exec, string, source, sourceRanges.data(), sourceRanges.size(), replacements.data(), replacements.size()));
}

static inline EncodedTiValue replaceUsingStringSearch(ExecState* exec, JSString* jsString, TiValue searchValue)
{
    ...
    if (matchStart == notFound)
        return TiValue::encode(jsString);
    ...
    return TiValue::encode(TI::jsString(exec, leftPart, middlePart, rightPart));
}

static inline bool checkObjectCoercible(TiValue thisValue)
{
...
EncodedTiValue JSC_HOST_CALL stringProtoFuncReplace(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    if (!checkObjectCoercible(thisValue))
        return throwVMTypeError(exec);
    JSString* string = thisValue.toString(exec);
    TiValue searchValue = exec->argument(0);
...
EncodedTiValue JSC_HOST_CALL stringProtoFuncToString(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
...
EncodedTiValue JSC_HOST_CALL stringProtoFuncCharAt(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    if (!checkObjectCoercible(thisValue))
        return throwVMTypeError(exec);
...
EncodedTiValue JSC_HOST_CALL stringProtoFuncCharCodeAt(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    if (!checkObjectCoercible(thisValue))
        return throwVMTypeError(exec);
    ...
    if (a0.isUInt32()) {
        ...
        if (i < len) {
            if (s.is8Bit())
                return TiValue::encode(jsNumber(s.characters8()[i]));
            return TiValue::encode(jsNumber(s.characters16()[i]));
        }
...
EncodedTiValue JSC_HOST_CALL stringProtoFuncConcat(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    if (thisValue.isString() && exec->argumentCount() == 1)
        return TiValue::encode(jsString(exec, asString(thisValue), exec->uncheckedArgument(0).toString(exec)));

    if (!checkObjectCoercible(thisValue))
        return throwVMTypeError(exec);
    return TiValue::encode(jsStringFromArguments(exec, thisValue));
}

EncodedTiValue JSC_HOST_CALL stringProtoFuncIndexOf(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    if (!checkObjectCoercible(thisValue))
        return throwVMTypeError(exec);
...
EncodedTiValue JSC_HOST_CALL stringProtoFuncLastIndexOf(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    if (!checkObjectCoercible(thisValue))
        return throwVMTypeError(exec);
...
EncodedTiValue JSC_HOST_CALL stringProtoFuncMatch(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    if (!checkObjectCoercible(thisValue))
        return throwVMTypeError(exec);
    ...
    if (!global)
        return TiValue::encode(result ? RegExpMatchesArray::create(exec, string, regExp, result) : jsNull());
    ...
    return TiValue::encode(constructArray(exec, static_cast<ArrayAllocationProfile*>(0), list));
}

EncodedTiValue JSC_HOST_CALL stringProtoFuncSearch(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    if (!checkObjectCoercible(thisValue))
        return throwVMTypeError(exec);
    ...
    return TiValue::encode(result ? jsNumber(result.start) : jsNumber(-1));
}

EncodedTiValue JSC_HOST_CALL stringProtoFuncSlice(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    if (!checkObjectCoercible(thisValue))
        return throwVMTypeError(exec);
...
static ALWAYS_INLINE bool splitStringByOneCharacterImpl(ExecState* exec, JSArray* result, const String& input, StringImpl* string, UChar separatorCharacter, size_t& position, unsigned& resultLength, unsigned limitLength)
{
    ...
    while ((matchPosition = WTI::find(characters, string->length(), separatorCharacter, position)) != notFound) {
...
EncodedTiValue JSC_HOST_CALL stringProtoFuncSplit(ExecState* exec)
{
    ...
    TiValue limitValue = exec->argument(1);
    ...
    TiValue separatorValue = exec->argument(0);
...
EncodedTiValue JSC_HOST_CALL stringProtoFuncSubstr(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    if (!checkObjectCoercible(thisValue))
        return throwVMTypeError(exec);
...
EncodedTiValue JSC_HOST_CALL stringProtoFuncSubstring(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    if (!checkObjectCoercible(thisValue))
        return throwVMTypeError(exec);
...
EncodedTiValue JSC_HOST_CALL stringProtoFuncToLowerCase(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    if (!checkObjectCoercible(thisValue))
        return throwVMTypeError(exec);
    ...
    if (ourImpl == lower)
        return TiValue::encode(sVal);
    return TiValue::encode(jsString(exec, String(lower.release())));
}

EncodedTiValue JSC_HOST_CALL stringProtoFuncToUpperCase(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    if (!checkObjectCoercible(thisValue))
        return throwVMTypeError(exec);
    ...
    if (sImpl == upper)
        return TiValue::encode(sVal);
    return TiValue::encode(jsString(exec, String(upper.release())));
}

EncodedTiValue JSC_HOST_CALL stringProtoFuncLocaleCompare(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    if (!checkObjectCoercible(thisValue))
        return throwVMTypeError(exec);
    ...
    TiValue a0 = exec->argument(0);
    return TiValue::encode(jsNumber(localeCompare(s, a0.toString(exec)->value(exec))));
}

EncodedTiValue JSC_HOST_CALL stringProtoFuncBig(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    if (!checkObjectCoercible(thisValue))
        return throwVMTypeError(exec);
...
EncodedTiValue JSC_HOST_CALL stringProtoFuncSmall(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    if (!checkObjectCoercible(thisValue))
        return throwVMTypeError(exec);
...
EncodedTiValue JSC_HOST_CALL stringProtoFuncBlink(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    if (!checkObjectCoercible(thisValue))
        return throwVMTypeError(exec);
...
EncodedTiValue JSC_HOST_CALL stringProtoFuncBold(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    if (!checkObjectCoercible(thisValue))
        return throwVMTypeError(exec);
...
EncodedTiValue JSC_HOST_CALL stringProtoFuncFixed(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    if (!checkObjectCoercible(thisValue))
        return throwVMTypeError(exec);
...
EncodedTiValue JSC_HOST_CALL stringProtoFuncItalics(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    if (!checkObjectCoercible(thisValue))
        return throwVMTypeError(exec);
...
EncodedTiValue JSC_HOST_CALL stringProtoFuncStrike(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    if (!checkObjectCoercible(thisValue))
        return throwVMTypeError(exec);
...
EncodedTiValue JSC_HOST_CALL stringProtoFuncSub(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    if (!checkObjectCoercible(thisValue))
        return throwVMTypeError(exec);
...
EncodedTiValue JSC_HOST_CALL stringProtoFuncSup(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    if (!checkObjectCoercible(thisValue))
        return throwVMTypeError(exec);
...
EncodedTiValue JSC_HOST_CALL stringProtoFuncFontcolor(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    if (!checkObjectCoercible(thisValue))
        return throwVMTypeError(exec);
    ...
    return TiValue::encode(jsMakeNontrivialString(exec, "<font color=\"", color, "\">", s, "</font>"));
}

EncodedTiValue JSC_HOST_CALL stringProtoFuncFontsize(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    if (!checkObjectCoercible(thisValue))
        return throwVMTypeError(exec);
    ...
    return TiValue::encode(jsMakeNontrivialString(exec, "<font size=\"", fontSize, "\">", s, "</font>"));
}

EncodedTiValue JSC_HOST_CALL stringProtoFuncAnchor(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    if (!checkObjectCoercible(thisValue))
        return throwVMTypeError(exec);
    ...
    return TiValue::encode(jsMakeNontrivialString(exec, "<a name=\"", anchor, "\">", s, "</a>"));
}

EncodedTiValue JSC_HOST_CALL stringProtoFuncLink(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    if (!checkObjectCoercible(thisValue))
        return throwVMTypeError(exec);
...
static inline TiValue trimString(ExecState* exec, TiValue thisValue, int trimKind)
{
    if (!checkObjectCoercible(thisValue))
...
EncodedTiValue JSC_HOST_CALL stringProtoFuncTrim(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    return TiValue::encode(trimString(exec, thisValue, TrimLeft | TrimRight));
}

EncodedTiValue JSC_HOST_CALL stringProtoFuncTrimLeft(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
    return TiValue::encode(trimString(exec, thisValue, TrimLeft));
}

EncodedTiValue JSC_HOST_CALL stringProtoFuncTrimRight(ExecState* exec)
{
    TiValue thisValue = exec->hostThisValue();
