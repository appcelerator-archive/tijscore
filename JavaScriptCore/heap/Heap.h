...
namespace TI {
    ...
    typedef std::pair<TiValue, WTI::String> ValueStringPair;
    ...
    public:
        ...
        static Heap* heap(const TiValue); // 0 for immediate values
        ...
        static void writeBarrier(const JSCell*, TiValue);
        ...
        JS_EXPORT_PRIVATE void protect(TiValue);
        JS_EXPORT_PRIVATE bool unprotect(TiValue); // True when the protect count drops to 0.
    ...
    inline Heap* Heap::heap(const TiValue v)
    {
        if (!v.isCell())
            return 0;
    ...
    inline void Heap::writeBarrier(const JSCell*, TiValue)
    {
        WriteBarrierCounters::countWriteBarrier();
    }
