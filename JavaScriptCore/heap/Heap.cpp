...
struct GCTimerScope {
    GCTimerScope(GCTimer* timer)
        : m_timer(timer)
        , m_start(WTI::monotonicallyIncreasingTime())
    {
    }
    ~GCTimerScope()
    {
        double delta = WTI::monotonicallyIncreasingTime() - m_start;
...
Heap::Heap(VM* vm, HeapType heapType)
    ...
    , m_vm(vm)
    , m_lastGCLength(0)
    , m_lastCodeDiscardTime(WTI::monotonicallyIncreasingTime())
...
void Heap::markRoots()
{
    ...
    double gcStartTime = WTI::monotonicallyIncreasingTime();
#endif
...
#endif
    MARK_LOG_MESSAGE2("\nNumber of live Objects after full GC %lu, took %.6f secs\n", visitCount, WTI::monotonicallyIncreasingTime() - gcStartTime);
#endif
...
void Heap::collect(SweepToggle sweepToggle)
{
    ...
    double lastGCStartTime = WTI::monotonicallyIncreasingTime();
    ...
    m_bytesAllocated = 0;
    double lastGCEndTime = WTI::monotonicallyIncreasingTime();
