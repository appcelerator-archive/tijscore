...
namespace TI {
    ...
    private:
        ...
        WTI::Bitmap<atomsPerBlock, WTI::BitmapAtomic> m_marks;
#else
        WTI::Bitmap<atomsPerBlock, WTI::BitmapNotAtomic> m_marks;
#endif
        OwnPtr<WTI::Bitmap<atomsPerBlock>> m_newlyAllocated;
    ...
    {
        return WTI::roundUpToMultipleOf<atomSize>(sizeof(MarkedBlock)) / atomSize;
    }
...
namespace WTI {
    ...
    struct MarkedBlockHash : PtrHash<TI::MarkedBlock*> {
        static unsigned hash(TI::MarkedBlock* const& key)
        {
            ...
            return reinterpret_cast<TI::Bits>(key) / TI::MarkedBlock::blockSize;
        }
    };

    template<> struct DefaultHash<TI::MarkedBlock*> {
