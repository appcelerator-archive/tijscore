...
ALWAYS_INLINE bool JSString::tryHashConsLock()
{
    ...
    if (!WTI::weakCompareAndSwap(&m_flags, currentFlags, newFlags))
        return false;

    WTI::memoryBarrierAfterLock();
    return true;
#else
...
ALWAYS_INLINE void SlotVisitor::internalAppend(void* from, TiValue* slot)
{
    // This internalAppend is only intended for visits to object and array backing stores.
    // as it can change the TiValue pointed to be the argument when the original TiValue
    ...
    if (m_shouldHashCons && cell->isString()) {
        ...
        if (string->shouldTryHashCons() && string->tryHashConsLock()) {
            ...
            if (addResult.isNewEntry)
                ...
                TiValue existingTiValue = addResult.iterator->value;
                if (value != existingTiValue)
                    jsCast<JSString*>(existingTiValue.asCell())->clearHashConsSingleton();
                *slot = existingTiValue;
