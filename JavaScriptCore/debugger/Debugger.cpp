...
bool Debugger::hasBreakpoint(SourceID sourceID, const TextPosition& position, Breakpoint *hitBreakpoint)
{
    ...
    TiValue exception;
    TiValue result = DebuggerCallFrame::evaluateWithCallFrame(m_currentCallFrame, breakpoints[i].condition, exception);
...
void Debugger::exception(CallFrame* callFrame, TiValue exception, bool hasHandler)
{
    if (m_isPaused)
        return;
