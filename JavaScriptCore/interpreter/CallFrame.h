...
namespace TI  {
    ...
    public:
        TiValue calleeAsValue() const { return this[JSStack::Callee].jsValue(); }
        ...
        TiValue exception() const { return vm().exception(); }
        ...
        TiValue argument(size_t argument)
        {
            if (argument >= argumentCount())
                 return jsUndefined();
            return getArgumentUnsafe(argument);
        }
        TiValue uncheckedArgument(size_t argument)
        {
            ASSERT(argument < argumentCount());
            return getArgumentUnsafe(argument);
        }
        void setArgument(size_t argument, TiValue value)
        {
            this[argumentOffset(argument)] = value;
        }

        static int thisArgumentOffset() { return argumentOffsetIncludingThis(0); }
        TiValue thisValue() { return this[thisArgumentOffset()].jsValue(); }
        void setThisValue(TiValue value) { this[thisArgumentOffset()] = value; }

        TiValue argumentAfterCapture(size_t argument);
        ...
        TiValue hostThisValue() { return thisValue(); }
    ...
    private:
        ...
        TiValue getArgumentUnsafe(size_t argIndex)
        {
