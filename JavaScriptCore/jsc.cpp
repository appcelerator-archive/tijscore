...
#if HAVE(READLINE)
// readline/history.h has a Function typedef which conflicts with the WTI::Function template from WTF/Forward.h
...
static EncodedTiValue JSC_HOST_CALL functionPrint(ExecState*);
static EncodedTiValue JSC_HOST_CALL functionDebug(ExecState*);
static EncodedTiValue JSC_HOST_CALL functionDescribe(ExecState*);
static EncodedTiValue JSC_HOST_CALL functionJSCStack(ExecState*);
static EncodedTiValue JSC_HOST_CALL functionGC(ExecState*);
#ifndef NDEBUG
static EncodedTiValue JSC_HOST_CALL functionReleaseExecutableMemory(ExecState*);
static EncodedTiValue JSC_HOST_CALL functionDumpCallFrame(ExecState*);
#endif
static EncodedTiValue JSC_HOST_CALL functionVersion(ExecState*);
static EncodedTiValue JSC_HOST_CALL functionRun(ExecState*);
static EncodedTiValue JSC_HOST_CALL functionLoad(ExecState*);
static EncodedTiValue JSC_HOST_CALL functionReadFile(ExecState*);
static EncodedTiValue JSC_HOST_CALL functionCheckSyntax(ExecState*);
static EncodedTiValue JSC_HOST_CALL functionReadline(ExecState*);
static EncodedTiValue JSC_HOST_CALL functionPreciseTime(ExecState*);
static EncodedTiValue JSC_HOST_CALL functionNeverInlineFunction(ExecState*);
static EncodedTiValue JSC_HOST_CALL functionNumberOfDFGCompiles(ExecState*);
static EncodedTiValue JSC_HOST_CALL functionTransferArrayBuffer(ExecState*);
static NO_RETURN_WITH_VALUE EncodedTiValue JSC_HOST_CALL functionQuit(ExecState*);

#if ENABLE(SAMPLING_FLAGS)
static EncodedTiValue JSC_HOST_CALL functionSetSamplingFlags(ExecState*);
static EncodedTiValue JSC_HOST_CALL functionClearSamplingFlags(ExecState*);
#endif
...
public:
    ...
    static Structure* createStructure(VM& vm, TiValue prototype)
    {
...
EncodedTiValue JSC_HOST_CALL functionPrint(ExecState* exec)
{
...
#ifndef NDEBUG
EncodedTiValue JSC_HOST_CALL functionDumpCallFrame(ExecState* exec)
{
...
EncodedTiValue JSC_HOST_CALL functionDebug(ExecState* exec)
{
    fprintf(stderr, "--> %s\n", exec->argument(0).toString(exec)->value(exec).utf8().data());
    return TiValue::encode(jsUndefined());
}

EncodedTiValue JSC_HOST_CALL functionDescribe(ExecState* exec)
{
...
EncodedTiValue JSC_HOST_CALL functionJSCStack(ExecState* exec)
{
    StringBuilder trace;
...
EncodedTiValue JSC_HOST_CALL functionGC(ExecState* exec)
{
...
#ifndef NDEBUG
EncodedTiValue JSC_HOST_CALL functionReleaseExecutableMemory(ExecState* exec)
{
    JSLockHolder lock(exec);
...
EncodedTiValue JSC_HOST_CALL functionVersion(ExecState*)
{
...
EncodedTiValue JSC_HOST_CALL functionRun(ExecState* exec)
{
    String fileName = exec->argument(0).toString(exec)->value(exec);
    Vector<char> script;
    if (!fillBufferWithContentsOfFile(fileName, script))
        return TiValue::encode(exec->vm().throwException(exec, createError(exec, "Could not open file.")));
    ...
    StopWatch stopWatch;
    stopWatch.start();
    evaluate(globalObject->globalExec(), jscSource(script.data(), fileName), TiValue(), &exception);
    stopWatch.stop();
...
EncodedTiValue JSC_HOST_CALL functionLoad(ExecState* exec)
{
    String fileName = exec->argument(0).toString(exec)->value(exec);
    Vector<char> script;
    if (!fillBufferWithContentsOfFile(fileName, script))
    ...
    TiValue evaluationException;
    TiValue result = evaluate(globalObject->globalExec(), jscSource(script.data(), fileName), TiValue(), &evaluationException);
    if (evaluationException)
        exec->vm().throwException(exec, evaluationException);
    return TiValue::encode(result);
}

EncodedTiValue JSC_HOST_CALL functionReadFile(ExecState* exec)
{
    String fileName = exec->argument(0).toString(exec)->value(exec);
    Vector<char> script;
    if (!fillBufferWithContentsOfFile(fileName, script))
    ...
    return TiValue::encode(jsString(exec, stringFromUTF(script.data())));
}

EncodedTiValue JSC_HOST_CALL functionCheckSyntax(ExecState* exec)
{
    String fileName = exec->argument(0).toString(exec)->value(exec);
    Vector<char> script;
    if (!fillBufferWithContentsOfFile(fileName, script))
...
#if ENABLE(SAMPLING_FLAGS)
EncodedTiValue JSC_HOST_CALL functionSetSamplingFlags(ExecState* exec)
{
    for (unsigned i = 0; i < exec->argumentCount(); ++i) {
...
EncodedTiValue JSC_HOST_CALL functionClearSamplingFlags(ExecState* exec)
{
    for (unsigned i = 0; i < exec->argumentCount(); ++i) {
        unsigned flag = static_cast<unsigned>(exec->uncheckedArgument(i).toNumber(exec));
        if ((flag >= 1) && (flag <= 32))
...
EncodedTiValue JSC_HOST_CALL functionReadline(ExecState* exec)
{
...
EncodedTiValue JSC_HOST_CALL functionPreciseTime(ExecState*)
{
    return TiValue::encode(jsNumber(currentTime()));
}

EncodedTiValue JSC_HOST_CALL functionNeverInlineFunction(ExecState* exec)
{
    return TiValue::encode(setNeverInline(exec));
}

EncodedTiValue JSC_HOST_CALL functionNumberOfDFGCompiles(ExecState* exec)
{
    return TiValue::encode(numberOfDFGCompiles(exec));
}

EncodedTiValue JSC_HOST_CALL functionTransferArrayBuffer(ExecState* exec)
{
    if (exec->argumentCount() < 1)
        return TiValue::encode(exec->vm().throwException(exec, createError(exec, "Not enough arguments")));
    
    JSArrayBuffer* buffer = jsDynamicCast<JSArrayBuffer*>(exec->argument(0));
    if (!buffer)
        return TiValue::encode(exec->vm().throwException(exec, createError(exec, "Expected an array buffer")));
...
EncodedTiValue JSC_HOST_CALL functionQuit(ExecState*)
{
    exit(EXIT_SUCCESS);
...
static bool runWithScripts(GlobalObject* globalObject, const Vector<Script>& scripts, bool dump)
{
    ...
    if (dump)
        TI::Options::dumpGeneratedBytecodes() = true;
    ...
    for (size_t i = 0; i < scripts.size(); i++) {
        ...
        TiValue evaluationException;
        TiValue returnValue = evaluate(globalObject->globalExec(), jscSource(script, fileName), TiValue(), &evaluationException);
        ...
        if (evaluationException) {
            ...
            TiValue stackValue = evaluationException.get(globalObject->globalExec(), stackID);
...
static void runInteractive(GlobalObject* globalObject)
{
    ...
    while (!shouldQuit) {
        ...
        TiValue evaluationException;
        TiValue returnValue = evaluate(globalObject->globalExec(), makeSource(source, interpreterName), TiValue(), &evaluationException);
#else
        ...
        TiValue evaluationException;
        TiValue returnValue = evaluate(globalObject->globalExec(), jscSource(line.data(), interpreterName), TiValue(), &evaluationException);
#endif
        if (evaluationException)
            printf("Exception: %s\n", evaluationException.toString(globalObject->globalExec())->value(globalObject->globalExec()).utf8().data());
        else
...
void CommandLine::parseArguments(int argc, char** argv)
{
    ...
    if (needToDumpOptions)
        TI::Options::dumpAllOptions(stderr);
    if (needToExit)
        exit(EXIT_SUCCESS);
}
