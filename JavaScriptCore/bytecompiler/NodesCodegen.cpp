...
TiValue StringNode::jsValue(BytecodeGenerator& generator) const
{
...
static inline bool canFoldToBranch(OpcodeID opcodeID, ExpressionNode* branchExpression, TiValue constant)
{
...
void BinaryOpNode::tryFoldToBranch(BytecodeGenerator& generator, TriState& branchCondition, ExpressionNode*& branchExpression)
{
    ...
    TiValue value = constant->jsValue(generator);
    bool canFoldToBranch = TI::canFoldToBranch(opcodeID, branchExpression, value);
    if (!canFoldToBranch)
        return;
...
{
    StatementNode* singleStatement = TI::singleStatement(ifBlock);
    if (!singleStatement)
        return false;
